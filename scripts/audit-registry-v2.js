...
assert(/href="\/registry-v2\.css\?(?:v|build)=[^"]+"/.test(html), 'Registry v2 stylesheet is not published.');
assert(/src="\/registry-v2\.js\?(?:v|build)=[^"]+"/.test(html), 'Registry v2 runtime is not published.');
...
assert(stylesheetLinks[2] === '/drx-dashboard-stripe.css?v=drx-dashboard-stripe-v8-responsive4', 'Shared Stripe shell v8 must load last.');
...
assert(scriptSources[2] === '/sidebar-taxonomy-v3.js?v=sidebar-taxonomy-v5-polish2-lazy', 'Shared sidebar taxonomy v5 must load before the registry runtime.');
...
console.log(JSON.stringify({
  ...
  shellVersion:'drx-dashboard-stripe-v8-responsive4',
  tableHeaderCount,
