...
function patchPerformanceDiagnostics() {
  ...
  const after = `    const idleMutationAudit = await page.evaluate(() => new Promise(resolve => {\n      const target = document.getElementById('tbody');\n      let count = 0;\n      const targets = new Map();\n      const operations = new Map();\n      const rowNumber = node => {\n        const row = node?.closest?.('tr');\n        return row?.dataset?.registryNumber || row?.querySelector?.('.drug-select')?.dataset?.registryNumber || '';\n      };\n      const rememberOperation = (name, node) => {\n        const row = node?.closest?.('tr');\n        if (!row || !target.contains(row)) return;\n        const stack = String(new Error().stack || '')\n          .split('\\n')\n          .slice(2, 6)\n          .map(line => line.trim())\n          .join(' <- ');\n        const key = [name, rowNumber(row), stack].join('|');\n        operations.set(key, (operations.get(key) || 0) + 1);\n      };\n      const originalReplaceWith = Element.prototype.replaceWith;\n      const originalReplaceChildren = Element.prototype.replaceChildren;\n      const originalAppendChild = Node.prototype.appendChild;\n      Element.prototype.replaceWith = function(...nodes) {\n        rememberOperation('replaceWith', this);\n        return originalReplaceWith.apply(this, nodes);\n      };\n      Element.prototype.replaceChildren = function(...nodes) {\n        rememberOperation('replaceChildren', this);\n        return originalReplaceChildren.apply(this, nodes);\n      };\n      Node.prototype.appendChild = function(node) {\n        if (this?.nodeType === Node.ELEMENT_NODE) rememberOperation('appendChild', this);\n        return originalAppendChild.call(this, node);\n      };\n      const observer = new MutationObserver(records => {\n        count += records.length;\n        records.forEach(record => {\n          const node = record.target?.nodeType === Node.TEXT_NODE ? record.target.parentElement : record.target;\n          const row = node?.closest?.('tr');\n          const cell = node?.closest?.('td');\n          const added = [...record.addedNodes].map(item => item?.dataset?.registryColumnKey || item?.dataset?.registryDosageColumn || item?.dataset?.registryDoseCalculatorColumn || item?.className || item?.nodeName || '').join(',');\n          const removed = [...record.removedNodes].map(item => item?.dataset?.registryColumnKey || item?.dataset?.registryDosageColumn || item?.dataset?.registryDoseCalculatorColumn || item?.className || item?.nodeName || '').join(',');\n          const key = [\n            record.type,\n            cell?.dataset?.registryColumnKey || cell?.dataset?.registryDosageColumn || cell?.dataset?.clinicalEditorColumn || cell?.className || '',\n            rowNumber(row),\n            'add:' + added,\n            'remove:' + removed,\n          ].join('|');\n          targets.set(key, (targets.get(key) || 0) + 1);\n        });\n      });\n      observer.observe(target, { childList:true, subtree:true, characterData:true });\n      setTimeout(() => {\n        observer.disconnect();\n        Element.prototype.replaceWith = originalReplaceWith;\n        Element.prototype.replaceChildren = originalReplaceChildren;\n        Node.prototype.appendChild = originalAppendChild;\n        resolve({\n          count,\n          targets:[...targets.entries()].sort((a,b) => b[1] - a[1]).slice(0,30),\n          operations:[...operations.entries()].sort((a,b) => b[1] - a[1]).slice(0,30),\n        });\n      }, 1500);\n    }));\n    console.log(\`REGISTRY_IDLE_MUTATIONS \${idleMutationAudit.count}\`);\n    console.log(\`REGISTRY_IDLE_MUTATION_TARGETS \${JSON.stringify(idleMutationAudit.targets)}\`);\n    console.log(\`REGISTRY_IDLE_MUTATION_OPERATIONS \${JSON.stringify(idleMutationAudit.operations)}\`);\n    expect(idleMutationAudit.count, 'dosage integration entered a DOM mutation feedback loop').toBeLessThanOrEqual(8);`;
  if (!source.includes('REGISTRY_IDLE_MUTATION_OPERATIONS')) {
    if (source.includes('REGISTRY_IDLE_MUTATION_TARGETS')) {
      const start = source.indexOf('    const idleMutationAudit = await page.evaluate');
      const endNeedle = "    expect(idleMutationAudit.count, 'dosage integration entered a DOM mutation feedback loop').toBeLessThanOrEqual(8);";
      const end = source.indexOf(endNeedle, start);
      if (start < 0 || end < 0) throw new Error('Phase 5 existing diagnostic block could not be replaced.');
      source = source.slice(0, start) + after + source.slice(end + endNeedle.length);
    } else {
