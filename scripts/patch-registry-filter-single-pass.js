...
if (!source.includes("formType === 'category' && categoryOf(row['Forma farmaceutike']) !== formValue")) {
  throw new Error('Registry category filtering semantics must be preserved.');
}

fs.writeFileSync(TARGET, source, 'utf8');
console.log('Registry search, status and pharmaceutical-form filters share one cached pass over the canonical dataset.');
