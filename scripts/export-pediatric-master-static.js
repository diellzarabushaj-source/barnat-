...
const EXPECTED_IN_REVIEW = 74;
const EXPECTED_NEEDS_SOURCE = 46;
...
const FIELDS = Object.freeze([
  ...
  'pediatric_min_weight_kg','pediatric_max_weight_kg','pediatric_dose_min','pediatric_dose_max',
  'pediatric_dose_unit','pediatric_dose_basis','pediatric_doses_per_day','pediatric_interval_hours',
  'pediatric_max_single_value','pediatric_max_single_unit','pediatric_max_daily_value','pediatric_max_daily_unit',
  'pediatric_route','pediatric_restriction','pediatric_concentration_value','pediatric_concentration_unit',
...
async function loadChunk(start, end) {
  ...
  const query = `drugs?select=${encodeURIComponent(select)}`
    ...
    + `&limit=${end - start + 1}`;
  const { data } = await neonRequest(query, { timeoutMs:15000, label:`Pediatric static export ${start}-${end}` });
  ...
  rows.forEach((row, index) => {
    ...
    if (cleanTsv(row.pediatric_primary_regimen_id) !== `card:${registry}:pediatric`) throw new Error(`Regimen mismatch for registry ${registry}.`);
  });
  return rows;
}
...
(async () => {
  ...
  if (total !== expectedTotal || lines.length !== expectedTotal) throw new Error(`Pediatric export row count mismatch: ${total}/${expectedTotal}.`);
  if (verified !== EXPECTED_VERIFIED || inReview !== EXPECTED_IN_REVIEW || needsSource !== EXPECTED_NEEDS_SOURCE) {
    throw new Error(`Pediatric export verification counts mismatch: verified=${verified}, in_review=${inReview}, needs_source=${needsSource}.`);
  }

  fs.writeFileSync(MASTER_FILE, lines.join('\n'), 'utf8');
  console.log(`Temporary tracked pediatric static export ready: ${total} rows, ${verified} verified, ${inReview} in_review, ${needsSource} needs_source, ${FIELDS.length} columns.`);
})().catch(error => {
