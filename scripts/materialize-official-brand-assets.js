...
const source = path.join(ROOT, 'brand/source-v2/medindex-mark-on-light-transparent.webp.b64');
const output = path.join(ROOT, 'brand/medindex-mark-on-light.webp');

if (!fs.existsSync(source)) throw new Error('Official transparent MedIndex logo source is missing.');
...
if (buffer.length < 1000 || buffer.toString('ascii', 0, 4) !== 'RIFF' || buffer.toString('ascii', 8, 12) !== 'WEBP') {
  throw new Error('Official transparent MedIndex logo source is invalid.');
}
fs.mkdirSync(path.dirname(output), { recursive:true });
fs.writeFileSync(output, buffer);
console.log(`Materialized transparent official MedIndex logo (${buffer.length} bytes).`);
...
console.log('Materialized transparent MedIndex PWA aliases from the approved mark.');
