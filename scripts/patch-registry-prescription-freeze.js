...
function patchUnifiedTable() {
  ...
  if (!source.includes("--registry-frozen-prescription-left")) {
    ...
    if (source.includes(mobileAnchor)) {
      source = source.replace(mobileAnchor, mobileAnchor + "      table.style.removeProperty('--registry-frozen-prescription-left');\n");
    }
...
function verify() {
  ...
  if (!unified.includes("'select', 'number', 'prescription-label', 'trade-name', 'active-substance'")
