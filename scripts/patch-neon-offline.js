...
const updated = source
  .replace("const PRIVATE_CACHE = 'medindex-private-resilient-v2';", "const PRIVATE_CACHE = 'medindex-private-neon-v1';")
  .replace(
  ...
  .replace("    ['medindex-private-', PRIVATE_CACHE],\n", '');

if (updated === source
    || !updated.includes("const PRIVATE_CACHE = 'medindex-private-neon-v1';")
    || !updated.includes("const QUERY_DATA_PATHS = new Set(['/api/drug-search', '/api/icd']);")
    || updated.includes("'/api/dosage', '/api/icd'")
    || updated.includes("['medindex-private-', PRIVATE_CACHE]")) {
  throw new Error('Service Worker-i nuk u patch-ua për cutover-in Neon dhe cache të ndarë ICD/labs.');
}
...
console.log('Patched Service Worker for Neon private-cache cutover and isolated ICD/laboratory query caches.');
