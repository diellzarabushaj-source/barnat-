...
const MARKER = 'registry-prescription-notation-display-v1';
...
function patchUnifiedTable() {
  ...
  const after = `    } else if (key === 'form') {\n      cell.className = 'wrap registry-form-cell';\n      cell.innerHTML = '<span class="cat-dot" aria-hidden="true"></span><span class="registry-cell-value"></span>';\n      cell.querySelector('.registry-cell-value').textContent = value || '—';\n    } else if (key === 'prescription-label') {\n      // ${MARKER}: never present a synthetic dash as if it were prescription\n      // content. Column-lite will also rehydrate this cell from the canonical row.\n      cell.className = 'wrap';\n      cell.textContent = value;\n    } else {\n      cell.textContent = value || '—';\n    }\n    cell.title = value;`;
  source = replaceOnce(source, before, after, 'unified prescription placeholder');
...
patchUnifiedTable();
verifyDataContract();
console.log('Prescription notation display fixed: canonical API value always rehydrates the visible cell; genuinely missing notation is blank, never a dash placeholder.');
