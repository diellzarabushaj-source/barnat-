(() => {
  ...
  function exactRegimenMatch(regimen, article) {
    ...
    if (!articleForm || !patterns.length || !patterns.some(pattern => articleForm === pattern || articleForm.includes(pattern) || pattern.includes(articleForm))) {
      return false;
    }
  ...
  function applyAutomatically(article, regimen) {
    ...
    if (preview) preview.innerHTML = `<b>Rp.</b>${[prefix, generic, strength].filter(Boolean).join(' ')}`;
  ...
  function maybeAutoApply(article) {
    ...
    if (hasManualClinicalContent(article)) return;

    const regimens = matchingRegimens(article);
    if (regimens.length !== 1) return;
  ...
  function run() {
    ...
    if (!dosage) {
      waitAttempts += 1;
      if (waitAttempts < 80) window.setTimeout(schedule, 150);
      return;
    }
    ...
    if (!notified && dosage.meta?.clinicalAutoFillEnabled === true) {
      notified = true;
      console.info(`MedIndex auto-apply ${VERSION} aktiv.`);
    }
  }
