(() => {
  ...
  function resetResult() { $('result').hidden = true; $('resultBody').textContent = ''; }

  function renderSubstances() {
    const select = $('substance');
    select.textContent = ''; select.append(option('', 'Zgjedh substancën aktive'));
  ...
  function renderRegimens() {
    ...
    select.textContent = ''; select.append(option('', 'Zgjedh indikacionin'));
  ...
  function selectRegimen() {
    ...
    if (!state.regimen) { $('patientFields').hidden = true; $('calculate').disabled = true; return; }
  ...
  function fact(label, value) {
    const row = document.createElement('div'); row.className = 'result-fact';
  ...
  function renderResult(result) {
    const body = $('resultBody'); body.textContent = '';
    body.append(
    ...
    const source = document.createElement('a'); source.href = result.source.url; source.target = '_blank'; source.rel = 'noopener noreferrer'; source.textContent = `Burimi · SmPC §${result.source.section}`;
    source.className = 'source-link'; body.append(source);
    const review = document.createElement('p'); review.className = 'review-note'; review.textContent = 'Kërkon verifikim klinik para përdorimit në recetë.'; body.append(review);
    $('result').hidden = false;
  }
  ...
  async function loadSubstance() {
    ...
    state.regimen = null; resetResult();
    if (!id) { state.regimens = []; renderRegimens(); return; }
  ...
  async function calculate() {
    ...
    $('calculate').disabled = true; $('status').textContent = 'Duke llogaritur…';
    try {
