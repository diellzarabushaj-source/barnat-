...
function assertPreviewRequest(req) {
  if (process.env.VERCEL_ENV !== 'preview') throw new Error('Ky endpoint lejohet vetëm në preview.');
  if (process.env.VERCEL_GIT_COMMIT_REF !== EXPECTED_BRANCH) throw new Error('Branch-i i preview-t nuk lejohet.');
  const guard = String(req.query?.guard || '');
  if (guard !== 'validated-sheet-revision-only') throw new Error('Mungon guard-i i aktivizimit.');
}
...
module.exports = async function handler(req, res) {
  ...
  if (req.method !== 'GET') return res.status(405).json({ ok:false, error:'Metoda nuk lejohet.' });
