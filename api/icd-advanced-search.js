...
function meta(dataset) {
  return {
    ...
    search:{
      ...
      supports:['code', 'sq-title', 'en-title', 'sq-synonym', 'typo', 'wildcard', 'hierarchy-groups'],
...
function tablePayload(dataset, query, counts) {
  ...
  rows = ranked.map(item => ({ ...item.node, searchMatch:item.match ? {
  ...
  const context = parent ? byCode.get(parent) || null : null;
...
function suggestionPayload(dataset, query, counts) {
  ...
  const result = Search.suggestDataset(dataset, q, { limit:18 });
  return {
    meta:meta(dataset),
    ...result,
    rows:result.rows.map(node => compactNode(node, counts)),
  };
}
