...
const SEARCH_SELECT = [
  'id','registry_number','pdid','trade_name','active_substance','atc_code','drug_class','use_text',
  'strength','pharmaceutical_form','packaging','product_status','retail_price'
].join(',');
...
function safeFilterText(value, max = 160) {
  return clean(value).slice(0, max).replace(/[,*%()\\]/g, ' ').replace(/\s+/g, ' ').trim();
}
...
function searchRow(row) {
  const substance = clean(row.active_substance);
  const tradeName = clean(row.trade_name);
  const strength = clean(row.strength);
  return {
    key:[clean(row.pdid), tradeName, strength].join('|'),
    id:clean(row.id),
    registryNumber:row.registry_number ?? null,
    pdid:clean(row.pdid),
    tradeName,
...
function buildPageRequest(query = {}) {
  ...
  const q = safeFilterText(query.q);
  const status = safeFilterText(query.status, 80);
  const form = safeFilterText(query.form, 120);
...
function buildDetailPath(query = {}) {
  const id = safeFilterText(query.id, 80);
...
function buildSearchPath(value) {
  const q = safeFilterText(value);
  if (q.length < 2) return null;
  const params = new URLSearchParams();
  params.set('select', SEARCH_SELECT);
  params.set('is_published', 'eq.true');
  params.set('editorial_status', 'eq.published');
  params.set('global_search_text', `ilike.*${q}*`);
  params.set('order', 'trade_name.asc.nullslast,registry_number.asc');
  params.set('limit', String(SEARCH_LIMIT));
  return { path:`drugs?${params.toString()}`, q };
}
...
async function sendPage(req, res, startedAt) {
  const request = buildPageRequest(requestQuery(req));
  const { data, response } = await supabaseRequest(request.path, {
    timeoutMs:6500,
    label:'Supabase registry page',
    ...(request.includeTotal ? { prefer:'count=exact' } : {}),
  });
  const rows = Array.isArray(data) ? data.map(listRow) : [];
  ...
  setHeaders(res, startedAt, 'supabase-registry-page');
  ...
  return res.status(200).json({
    ...
    query:{ q:request.q, status:request.status, form:request.form, sort:request.sort, direction:request.direction, includeTotal:request.includeTotal },
    meta:{ source:'supabase' },
  });
}

async function sendDetail(req, res, startedAt) {
  const path = buildDetailPath(requestQuery(req));
  if (!path) return res.status(400).json({ error:'ID e barit është e pavlefshme.' });
  const { data } = await supabaseRequest(path, { timeoutMs:5000, label:'Supabase registry detail' });
  const row = Array.isArray(data) && data.length ? detailRow(data[0]) : null;
  setHeaders(res, startedAt, 'supabase-registry-detail');
  if (req.method === 'HEAD') return res.status(row ? 200 : 404).end();
  return row ? res.status(200).json({ ok:true, row, meta:{ source:'supabase' } }) : res.status(404).json({ error:'Bari nuk u gjet.' });
}

async function sendSearch(req, res, startedAt) {
  const request = buildSearchPath(requestQuery(req).q);
  setHeaders(res, startedAt, 'supabase-drug-search');
  if (!request) return req.method === 'HEAD' ? res.status(200).end() : res.status(200).json({ ok:true, query:'', results:[], meta:{ source:'supabase' } });
  const { data } = await supabaseRequest(request.path, { timeoutMs:5000, label:'Supabase drug search' });
  const results = Array.isArray(data) ? data.map(searchRow) : [];
  if (req.method === 'HEAD') return res.status(200).end();
  return res.status(200).json({ ok:true, query:request.q, results, meta:{ source:'supabase' } });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return await sendSearch(req, res, startedAt);
  } catch (error) {
...
module.exports.buildPageRequest = buildPageRequest;
module.exports.buildDetailPath = buildDetailPath;
module.exports.buildSearchPath = buildSearchPath;
