...
function safeFilterText(value,max=160) { return clean(value).slice(0,max).replace(/[,*%()\\]/g,' ').replace(/\s+/g,' ').trim(); }
async function authorized(req) { const auth=await import('../lib/auth.mjs'); return auth.verifySessionToken(auth.sessionFromRequest(req)); }
...
function buildPageRequest(query={}) {
  ...
  const q=safeFilterText(query.q), status=safeFilterText(query.status,80), form=safeFilterText(query.form,120), sortKey=clean(query.sort).toLowerCase();
