...
function buildPageRequest(query={}) {
  ...
  const q=safeQueryText(query.q), status=safeFilterText(query.status,80), form=safeFilterText(query.form,120), formExact=safeExactForm(query.formExact), formCategory=safeExactForm(query.formCategory), sortKey=clean(query.sort).toLowerCase();
  ...
  return {path:`drugs?${params.toString()}`,page,pageSize,q,status,form,formExact,formCategory,sort:sortKey||'registry',direction,includeTotal};
}
