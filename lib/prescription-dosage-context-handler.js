...
const Dosage = require('./dosage-handler.js');
const PrescriptionDosage = require('./prescription-dosage-handler.js');
...
function requestDrugId(req) {
  const direct = clean(req?.query?.id);
  if (direct) return direct;
  try {
    return clean(new URL(req?.url || '/api/dosage', 'http://medindex.local').searchParams.get('id'));
  } catch {
    return '';
  }
}
...
async function handler(req, res) {
  try {
    ...
    let sourcePayload;
    if (drugId) {
      if (!PrescriptionDosage._test.UUID_RE.test(drugId)) {
        res.setHeader('Cache-Control', 'no-store');
        return res.status(400).json({
          code:'INVALID_DRUG_ID',
          error:'ID e barit është e pavlefshme.',
          forms:[], adult:[], pediatric:[], cards:[],
        });
      }
      const targeted = await PrescriptionDosage.loadDrugRegimens(drugId);
      sourcePayload = {
        forms:[],
        adult:targeted.adult,
        pediatric:targeted.pediatric,
        cards:[],
        meta:{ dataSource:'supabase', targeted:true, drugId, regimenRows:targeted.rows.length },
      };
    } else {
      const source = await Dosage.getPayload();
      sourcePayload = source.payload;
    }
    const payload = contextualize(sourcePayload, context);
    ...
    if (drugId) res.setHeader('X-MedIndex-Drug-ID', drugId);
...
handler._test = Object.freeze({ parseContext, regimenAdministration, routeMatches, prepareRegimen, contextualize, requestDrugId });
