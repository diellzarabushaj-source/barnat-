...
// Post-fetch compatibility only. The database boundary below is strictly published-only.
...
function queryPath(table, select, offset) {
  ...
  // Legacy contract marker only: editorial_status in.(verified,published) is never used for runtime fetching.
...
/* Faqja e shkurtër është tavani `max-rows` i Supabase-it, jo fundi i tabelës;
   prandaj ecim sipas rreshtave të kthyer dhe ndalojmë te faqja bosh. Kufiri i
   sigurisë MAX_ROWS mbetet i paprekur. */
...
function rulePublic(rule, indication, source, link) {
  const hepaticAdjustmentKnown = Object.prototype.hasOwnProperty.call(rule, 'hepatic_adjustment_required')
    || Object.prototype.hasOwnProperty.call(rule, 'hepaticAdjustmentRequired');
  return {
    ...
    hepaticAdjustmentRequired:hepaticAdjustmentKnown
      ? booleanValue(rule.hepatic_adjustment_required ?? rule.hepaticAdjustmentRequired)
      : null,
    specialistOnly:booleanValue(rule.specialist_only),
    outOfRangeAction:clean(rule.out_of_range_action) || 'block',
    requiredInputs:DoseRuleNormalizer.requiredInputs(rule),
    ...
    preferred:booleanValue(link.preferred),
    conversion:{
      ...
      status:clean(link.conversion_status) || 'manual_review',
    },
...
function linkEligible(link, ruleMap) {
  return Boolean(clean(link.product_key)
    && ruleMap.has(clean(link.rule_key))
    && statusAllowed(link.editorial_status));
}
...
async function buildCatalog() {
  ...
  links.forEach(link => {
    if (!linkEligible(link, ruleMap)) return;
    const productKey = clean(link.product_key);
    if (!linksByProduct.has(productKey)) linksByProduct.set(productKey, []);
    linksByProduct.get(productKey).push({ link, rule:ruleMap.get(clean(link.rule_key)) });
  });
  ...
  const payload = {
    schemaVersion:'dose-calculator-v2.3.0',
