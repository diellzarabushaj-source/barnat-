...
const EXPECTED_MINIMUMS = Object.freeze({
  drugs:3500,
  dosageRegimens:1400,
  icdCodes:650,
  labTests:100,
...
function shouldReadNeon() {
  return dataSourceMode() !== 'sheets';
}

function allowsSheetsFallback() {
...
async function fetchPaged(table, options = {}) {
  ...
  if (minimum && rows.length < minimum) {
    throw new Error(`${table}: Neon ktheu ${rows.length} rreshta; pritej së paku ${minimum}.`);
  }
  return rows;
}
...
async function getPublishedDrugRecords() {
  return fetchPaged('drugs', {
    ...
    filters:{ is_published:'eq.true', editorial_status:'eq.published' },
    order:'registry_number.asc',
    minimum:EXPECTED_MINIMUMS.drugs,
    maximum:6000,
  });
}

async function getPublishedDrugs() {
  return (await getPublishedDrugRecords()).map(mapDrugRow);
}

function regimenGroup(row) {
  return clean(row.source_key).split(':')[0].toLowerCase();
}

function adultRegimen(row) {
  const result = {
    regimenId:clean(row.regimen_code || row.source_key),
    substance:clean(row.active_substance),
    atc:clean(row.atc_code),
    form:clean(row.pharmaceutical_form),
    referenceStrength:clean(row.reference_strength),
    indication:clean(row.indication_text),
    icd:'',
    population:'adult',
    doseMg:clean(row.dose_text),
    practicalUnit:'',
    unitCount:'',
    route:clean(row.route),
    frequency:clean(row.frequency_text),
    intervalHours:numberOrNull(row.interval_hours),
    duration:clean(row.duration_text),
    prn:false,
    prnIndication:'',
    maxSingleMg:numberOrNull(row.max_single_mg),
    max24hMg:numberOrNull(row.max_daily_mg),
    maxUnits24h:'',
    dispense:'',
    signatura:clean(row.signatura_text || row.signatura_template),
    warnings:clean(row.warnings),
    renalHepatic:'',
    sourceUrl:/^https:\/\//i.test(clean(row.source_url)) ? clean(row.source_url) : '',
    sourceDate:row.reviewed_at || '',
    status:'VERIFIKUAR',
  };
...
function pediatricRegimen(row) {
  ...
  const value = numberOrNull(row.dose_value_min);
  const result = {
    regimenId:clean(row.regimen_code || row.source_key),
    substance:clean(row.active_substance),
    atc:clean(row.atc_code),
    form:clean(row.pharmaceutical_form),
    concentration:clean(row.reference_strength),
    indication:clean(row.indication_text),
    icd:'',
    minAgeMonths:numberOrNull(row.min_age_months),
    maxAgeMonths:numberOrNull(row.max_age_months),
    minWeightKg:numberOrNull(row.min_weight_kg),
    maxWeightKg:numberOrNull(row.max_weight_kg),
    regimenType:type,
    mgPerKg:type === 'mg_per_kg_dose' || type === 'mg_per_kg_day' ? value : null,
    basis:type === 'mg_per_kg_day' ? 'ditë' : type === 'mg_per_kg_dose' ? 'dozë' : '',
    dosesPerDay:numberOrNull(row.doses_per_day),
    ...
    duration:clean(row.duration_text),
    dispense:'',
    formula:clean(row.formula_text),
    signatura:clean(row.signatura_text || row.signatura_template),
    warnings:clean(row.warnings),
    sourceUrl:/^https:\/\//i.test(clean(row.source_url)) ? clean(row.source_url) : '',
    sourceDate:row.reviewed_at || '',
...
function cardFromPair(drug, adult, pediatric) {
  ...
  const sourceList = [...new Set([
    ...sourceUrls(adult?.source_url),
    ...sourceUrls(pediatric?.source_url),
  ])];
  return {
    cardKey:[clean(drug.pdid), clean(drug.trade_name), clean(drug.strength)].join('|'),
    ...
    strength:clean(drug.strength),
    drugClass:clean(drug.drug_class || source['Klasa / Çka është']),
    use:clean(drug.use_text || source['Përdorimi (fjalë kyçe)']),
    adultDose:clean(adult?.dose_text),
    adultRoute:clean(adult?.route),
    pediatricDose:clean(pediatric?.dose_text),
    pediatricRoute:clean(pediatric?.route),
    sourceUrls:sourceList,
    auditedAt:adult?.reviewed_at || pediatric?.reviewed_at || '',
    auditNote:clean(adult?.warnings || pediatric?.warnings),
    status:'VERIFIKUAR',
  };
}
...
async function getPublishedDosageRegimens() {
  const [regimens, drugs] = await Promise.all([
    fetchPaged('dosage_regimens', {
      ...
      minimum:EXPECTED_MINIMUMS.dosageRegimens,
      maximum:2500,
    }),
  ...
  const pediatric = regimens.filter(row => regimenGroup(row) === 'pediatric' && row.population === 'pediatric' && row.calculation_status === 'calculable_verified').map(pediatricRegimen);
  ...
  const cards = drugs
    .filter(drug => byDrug.has(drug.id))
  ...
  return {
    forms:formRowsFromDrugs(drugs),
    adult:adults,
    pediatric,
    cards,
    rawCount:regimens.length,
  };
}
...
function mapIcdRow(row) {
  ...
  return {
    code:clean(row.code),
    title:clean(row.title_sq),
    englishTitle:clean(row.title_en),
    level:/kategori/i.test(clean(row.level_name)) ? 'kategori' : 'kod',
    sourceLevel:clean(row.level_name),
    chapter:clean(row.chapter_code),
    chapterRange:'',
    chapterTitle:clean(row.chapter_title),
    group:clean(row.group_name),
    primaryCare:row.is_family_medicine ? 'E rëndësishme' : '',
    emergency:row.is_emergency ? 'Shumë i rëndësishëm' : '',
    priority,
    summary:clean(row.typical_use || row.description_sq),
    keywords:Array.isArray(row.tags) ? row.tags.map(clean).filter(Boolean) : [],
    warning:clean(row.warning_text),
    sourceUrl:/^https:\/\//i.test(clean(row.source_url)) ? clean(row.source_url) : '',
    codingNotes:[clean(row.coding_note)].filter(Boolean),
    includes:[],
    excludes:[],
    parent:clean(row.group_name || row.chapter_code),
    isFamilyMedicine:row.is_family_medicine === true,
...
async function getPublishedIcdCodes() {
  const rows = await fetchPaged('icd_codes', {
    ...
    filters:{ is_published:'eq.true', editorial_status:'eq.published' },
    order:'code.asc',
    minimum:EXPECTED_MINIMUMS.icdCodes,
    maximum:1500,
  });
...
async function getPublishedLabTests() {
  const [categories, tests] = await Promise.all([
    fetchPaged('lab_categories', {
      select:'id,category_number,title,description',
    ...
    fetchPaged('lab_tests', {
      ...
      filters:{ is_published:'eq.true', editorial_status:'eq.published' },
      order:'form_name.asc',
      minimum:EXPECTED_MINIMUMS.labTests,
      maximum:500,
    }),
  ]);
  ...
  const mappedCategories = categories.map(category => ({
    id:`category-${category.category_number}`,
    number:Number(category.category_number),
    label:`Kategoria ${category.category_number}`,
  ...
  const mappedTests = tests.map((test, index) => {
    ...
    return {
      id:clean(test.id) || `lab-${index + 1}`,
      categoryId:categoryIds.get(test.category_id) || '',
      category:category ? `Kategoria ${category.category_number} – ${clean(category.title)}` : '',
      analysis:`Analiza ${index + 1}`,
      formName:clean(test.form_name),
      englishName:clean(test.full_name_en),
      albanianName:clean(test.full_name_sq),
      whatItShows:clean(test.what_it_shows),
      highPositiveAbnormal:clean(test.high_when),
      lowNegativeNormal:clean(test.low_when),
  ...
  return {
    ...
    generatedAt:new Date().toISOString(),
    headers:{
      highPositiveAbnormal:'Kur rritet / rezulton pozitive / gjetje jonormale',
      lowNegativeNormal:'Kur ulet / rezulton negative / gjetje normale',
    },
