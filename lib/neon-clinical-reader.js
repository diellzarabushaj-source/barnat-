...
function dataSourceMode() {
  const value = clean(process.env.MEDINDEX_DATA_SOURCE || 'supabase').toLowerCase();
  if (value === 'supabase' || value === 'neon') return 'supabase';
  if (value === 'sheets') return 'sheets';
  if (value === 'hybrid') return 'hybrid';
  return 'supabase';
}
// Compatibility names are retained for callers, but the database transport is Supabase.
...
async function fetchPaged(table, options = {}) {
  ...
  const rows = [];
  /* Faqja e shkurtër është tavani `max-rows` i serverit, jo fundi i tabelës. */
...
function mapIcdRow(row) {
  ...
  return {
    code:clean(row.code), title:clean(row.title_sq), englishTitle:clean(row.title_en), latinTitle:clean(row.title_la),
...
async function getPublishedIcdCodes() {
  const rows = await fetchPaged('icd_codes', {
    select:'code,title_sq,title_en,title_la,description_sq,chapter_code,chapter_title,priority_level,is_family_medicine,is_emergency,tags,level_name,group_name,typical_use,warning_text,coding_note,is_critical,source_url,updated_at',
...
async function getPublishedLabTests() {
  const [categories, tests, indications, indicationTests] = await Promise.all([
    ...
    fetchPaged('lab_indications', {
      select:'id,slug,title_sq,title_en,summary_sq,icd_codes,aliases_sq,catalog_gaps,sort_order',
      filters:{ is_published:'eq.true', editorial_status:'eq.published' },
      order:'sort_order.asc',
      maximum:250,
    }),
    fetchPaged('lab_indication_tests', {
      select:'indication_id,lab_test_id,tier,rationale_sq,context_note_sq,sort_order',
      order:'sort_order.asc',
      maximum:3000,
    }),
  ]);
  ...
  const linksByIndication = new Map();
  indicationTests.forEach(link => {
    if (!linksByIndication.has(link.indication_id)) linksByIndication.set(link.indication_id, []);
    linksByIndication.get(link.indication_id).push({
      testId:clean(link.lab_test_id),
      tier:clean(link.tier) || 'recommended',
      rationale:clean(link.rationale_sq),
      contextNote:clean(link.context_note_sq),
      sortOrder:Number(link.sort_order) || 100,
    });
  });

  const mappedIndications = indications.map(indication => ({
    id:clean(indication.id),
    slug:clean(indication.slug),
    title:clean(indication.title_sq),
    titleEn:clean(indication.title_en),
    summary:clean(indication.summary_sq),
    icdCodes:Array.isArray(indication.icd_codes) ? indication.icd_codes.map(clean).filter(Boolean) : [],
    aliases:Array.isArray(indication.aliases_sq) ? indication.aliases_sq.map(clean).filter(Boolean) : [],
    catalogGaps:Array.isArray(indication.catalog_gaps) ? indication.catalog_gaps : [],
    sortOrder:Number(indication.sort_order) || 100,
    tests:(linksByIndication.get(indication.id) || []).sort((a, b) => a.sortOrder - b.sortOrder),
  }));

  return {
    source:'Supabase',
    sourceUrl:'',
    sourceNote:'Analizat dhe profilet klinike lexohen nga Supabase. Sugjerimet e panelit duhet të përshtaten me pacientin, qëllimin diagnostik dhe protokollet lokale.',
