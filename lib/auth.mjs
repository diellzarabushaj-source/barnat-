...
// Both Supabase doors — Google and email+password — produce the same kind of
// session and are held to the same rules: a verified auth UUID, an approved role
// and an active profile. Anything weaker is a legacy rollback session.
export const SUPABASE_PROVIDERS = Object.freeze(['supabase-google', 'supabase-password']);
const SESSION_PROVIDERS = Object.freeze([...SUPABASE_PROVIDERS, 'legacy-password', 'legacy-session']);
...
function normalizedIdentity(identity = {}) {
  ...
  const provider = SESSION_PROVIDERS.includes(String(identity.provider || ''))
...
export function isSupabaseSession(session) {
  return Boolean(session && session.v === SESSION_VERSION && SUPABASE_PROVIDERS.includes(session.provider) && session.authUid && session.authStatus === 'active');
}
