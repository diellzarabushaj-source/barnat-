...
function indexedCandidateNodes(dataset, rawQuery, { maxCandidates = 1400 } = {}) {
  ...
  const threshold = Math.max(1, Math.floor(evidenceUnits * 0.18));
  ...
  // Correctness wins over narrowing when the index evidence is weak.
  return ranked.length >= 24 ? ranked : nodes;
}
