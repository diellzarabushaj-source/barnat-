...
function bestMatch(node, rawQuery) {
  ...
  const codeIntent = canonicalCodeQuery(rawQuery).codeLike;
  const levelBoost = codeIntent
    ? (node?.level === 'subcategory' ? 18 : node?.level === 'category' ? 12 : 0)
    : (node?.level === 'category' ? 28 : node?.level === 'subcategory' ? 8 : 0);
