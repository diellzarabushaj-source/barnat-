...
function maximumReadRows() {
  const configured = Number(process.env.MEDINDEX_DATA_API_MAX_READ_ROWS || process.env.MEDINDEX_NEON_MAX_READ_ROWS);
...
function assertEgressSafeRead(path, options = {}) {
  const method = String(options.method || 'GET').toUpperCase();
  if (method !== 'GET' || envFlag('MEDINDEX_ALLOW_BULK_DATA_API_READS') || envFlag('MEDINDEX_ALLOW_BULK_NEON_READS')) return;
