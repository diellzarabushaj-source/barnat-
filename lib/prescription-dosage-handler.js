...
async function handler(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(data)) throw new Error('Neon nuk ktheu listë të dozologjisë së recetës.');
    ...
    return res.status(200).json({
      ...
      meta:{ dataSource:'neon', targeted:true, regimenRows:data.length, maxRegimens:MAX_REGIMENS },
    });
  } catch (error) {
    console.error('Targeted prescription dosage error:', error);
    res.setHeader('Cache-Control', 'no-store');
    if (/timed out|token is not available|Neon Data API/i.test(String(error?.message || ''))) {
