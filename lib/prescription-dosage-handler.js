...
async function loadDrugRegimens(drugId) {
  const id = clean(drugId);
  if (!UUID_RE.test(id)) {
    const error = new Error('ID e barit është e pavlefshme.');
    error.code = 'INVALID_DRUG_ID';
    throw error;
  }
  const { data } = await neonRequest(regimenPath(id), {
    timeoutMs:4500,
  ...
  return { drugId:id, rows:data, adult:mapped.adult, pediatric:mapped.pediatric };
}
...
async function handler(req, res) {
  ...
  try {
    ...
    const targeted = await loadDrugRegimens(drugId);

    setHeaders(res, startedAt);
    if (req.method === 'HEAD') return res.status(200).end();
    return res.status(200).json({
      ok:true,
      drugId:targeted.drugId,
      adult:targeted.adult,
      pediatric:targeted.pediatric,
      meta:{ dataSource:'supabase', targeted:true, regimenRows:targeted.rows.length, maxRegimens:MAX_REGIMENS },
    });
  } catch (error) {
...
handler.loadDrugRegimens = loadDrugRegimens;
