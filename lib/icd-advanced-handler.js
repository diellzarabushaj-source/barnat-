...
function compactNode(node, dataset, options = {}) {
  ...
  return {
    ...
    latinTitle:node.latinTitle || FullIcd.LATIN_TITLE_BY_CODE?.get(node.code) || '',
...
function send(req, res, payload, loaded, startedAt) {
  ...
  res.setHeader('X-MedIndex-Search-Version', 'sq-clinical-search-v2');
  res.setHeader('X-MedIndex-Search-Engine', 'clinical-ranking-v4');
