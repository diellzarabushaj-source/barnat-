...
function meta(dataset, loaded = {}) {
  return {
    ...
    search:{
      version:'sq-clinical-search-v5',
      engine:'clinical-ranking-v7',
      supports:[
  ...
  for (const candidate of intent.candidates || []) {
    ...
    rows.push({
      ...
      searchMatch:{
        ...
        field:'symptom',
        score:900 + Math.round(Number(candidate.weight || 0) * 100),
      ...
      symptomRelation:{
        ...
        retrievalWeight:Number(candidate.weight || 0),
...
function send(req, res, payload, loaded, startedAt) {
  ...
  res.setHeader('X-MedIndex-Search-Version', 'sq-clinical-search-v5');
  res.setHeader('X-MedIndex-Search-Engine', 'clinical-ranking-v7');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const view = requestedView === 'suggest' ? 'suggest' : requestedView === 'seed' ? 'seed' : 'table';
