...
/* PRN safety overlay mbi klasifikuesin e ngrirë pre-PRN. */
...
function adaptRowForPrn(row = {}) {
  ...
  if (!canCreateDailyEnvelope) return { row, schedule, synthetic:false };
  return {
...
function classify(row = {}) {
  ...
  if (CONTINUOUS_RATE_BASIS.has(basis)
    ...
    reasons.push('Kufijtë PRN të administrimeve nuk vlejnë për një infuzion të vazhdueshëm.');
  }
