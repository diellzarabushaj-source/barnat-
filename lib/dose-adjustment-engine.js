...
function validateAdjustmentRow(row = {}) {
  ...
  const sourceSectionSha256 = clean(row.sourceSectionSha256 ?? row.source_section_sha256);
  ...
  if (!/^[0-9a-f]{64}$/i.test(sourceSectionSha256)) errors.push('source_section_sha256_missing_or_invalid');
  if (!/^[0-9a-f]{64}$/i.test(sourceSnapshotId)) errors.push('source_snapshot_missing_or_invalid');
  if (!/^[0-9a-f]{64}$/i.test(sourceEvidenceHash)) errors.push('source_evidence_hash_missing_or_invalid');
  if (
    /^[0-9a-f]{64}$/i.test(sourceSnapshotId)
    && /^[0-9a-f]{64}$/i.test(sourceEvidenceHash)
    && sourceSnapshotId.toLowerCase() !== sourceEvidenceHash.toLowerCase()
  ) {
    errors.push('source_snapshot_evidence_hash_mismatch');
  }
  if (!['verified','approved'].includes(reviewStatus)) errors.push('review_status_not_verified');
  if (['verified','approved'].includes(reviewStatus)) {
...
function applyAdjustment(rule = {}, selection = {}) {
  ...
  if (a.doseAction === 'max_daily_cap') {
    ...
    // A safety cap may tighten an existing ceiling, never raise it.
