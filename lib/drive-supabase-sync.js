...
function validateSourceConfig(source, config) {
  if (!source || !config) throw new Error('Konfigurimi i sinkronizimit mungon.');
  const sourceScope = clean(source.entity_scope);
  const sourceKeyColumn = clean(source.key_column);
  if (sourceScope !== config.entityScope || sourceKeyColumn !== config.keyColumn) {
    throw new Error(
      `Konfigurimi i burimit nuk përputhet: pritej ${config.entityScope}/${config.keyColumn}, `
      + `u gjet ${sourceScope || 'pa-scope'}/${sourceKeyColumn || 'pa-key'}.`
    );
  }
  return true;
}
...
function requireMappedRow(item, record, label) {
  if (record) return record;
  throw new Error(`Staged ${label} row nuk kaloi validimin canonical: ${clean(item?.rowKey) || 'pa-key'}.`);
}

function requireMappedRows(item, records, label) {
  if (Array.isArray(records) && records.length) return records;
  throw new Error(`Staged ${label} row nuk prodhoi asnjë rekord canonical: ${clean(item?.rowKey) || 'pa-key'}.`);
}
...
async function syncNormalized(config, rows, deletedKeys) {
  ...
  if (config.entityScope === 'drugs') {
    const records = rows.map(item => requireMappedRow(item, mapDrug(item.values), 'drug'));
  ...
  if (config.entityScope === 'dosage_cards') {
    ...
    const records = rows.flatMap(item => {
      ...
      return requireMappedRows(item, mapCardDosages(item.values, ids.get(nr)), 'dosage-card');
    });
  ...
  if (config.entityScope === 'adult_dosage' || config.entityScope === 'pediatric_dosage') {
    ...
    const records = rows.map(item => requireMappedRow(item, mapper(item.values), config.entityScope));
    Object.assign(result, await upsertRows('dosage_regimens', records, ['source_key'], 'source_key'));
  ...
  if (config.entityScope === 'icd_codes') {
    const records = rows.map(item => requireMappedRow(item, mapIcd(item.values), 'icd'));
  ...
  if (config.entityScope === 'lab_tests') {
    ...
    for (const item of rows) {
      ...
      records.push(requireMappedRow(item, record, 'lab'));
    }
...
function normalizeRequestRows(config, rows) {
  ...
  return rows.flatMap(item => {
    ...
    const valueKey = clean(values?.[config.keyColumn]);
    const suppliedKey = clean(item?.rowKey);
    const rowKey = suppliedKey || valueKey;
    const rowNumber = numberOrNull(item?.rowNumber);
    if (!rowKey || !rowNumber || rowNumber <= config.headerRow) return [];

    if (suppliedKey && valueKey && suppliedKey !== valueKey) {
      throw new Error(`Row key nuk përputhet me ${config.keyColumn}: ${suppliedKey} != ${valueKey}.`);
    }
    if (seen.has(rowKey)) throw new Error(`Row key i dyfishuar në të njëjtin batch: ${rowKey}.`);
    seen.add(rowKey);
    ...
    return [{
      ...
      // Never trust a client-provided hash for reconciliation identity.
      sourceHash:rowHash(values),
    }];
  });
}

function normalizeDeletedKeys(values, rows) {
  const keys = [...new Set((Array.isArray(values) ? values : []).map(clean).filter(Boolean))]
    .slice(0, MAX_DELETED_KEYS);
  const stagedKeys = new Set((Array.isArray(rows) ? rows : []).map(row => clean(row?.rowKey)).filter(Boolean));
  const conflict = keys.find(key => stagedKeys.has(key));
  if (conflict) throw new Error(`I njëjti row key nuk mund të përditësohet dhe fshihet në të njëjtin batch: ${conflict}.`);
  return keys;
}
...
async function handle(req, res) {
  ...
  try {
    ...
    const deletedKeys = normalizeDeletedKeys(payload.deletedKeys, rows);
    ...
    validateSourceConfig(source, config);
    ...
    console.error('Drive to Supabase sync failed:', error);
