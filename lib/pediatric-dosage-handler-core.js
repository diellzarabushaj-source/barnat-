...
const { neonRequest, supabaseRequest } = require('./medindex-data-api.js');
const { STATUS, classify } = require('./pediatric-readiness.js');
const { calculate } = require('./pediatric-calculation.js');
const V3Calculator = require('./pediatric-v3-runtime.js');
...
async function calculateLegacyDose(body) {
  ...
  const outcome = await loadLegacyProduct(body?.drugId ?? body?.registryNumber);
...
async function loadPhase9Context(drugId) {
  if (!/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(clean(drugId))) return null;
  // Unit tests may intentionally provide a narrow Data API mock with only
  // neonRequest. Phase 9 context is enrichment, not a dosing authority, so a
  // missing privileged helper must degrade to no context rather than emit
  // misleading runtime noise or alter the verified dosing fallback.
  if (typeof supabaseRequest !== 'function') return null;
  const { data } = await supabaseRequest(
    'rpc/drx_phase9_product_context_v1',
    {
      method:'POST',
      body:{ p_drug_id:drugId },
      timeoutMs:4000,
      label:'DRx Phase 9 product context',
    },
    { privileged:true },
  );
...
async function loadLegacyProduct(rawSelector) {
  ...
  let phase9Context = null;
  try {
    phase9Context = await loadPhase9Context(drugId);
  } catch (error) {
    console.error('Phase 9 product context read failed:', error.message);
  }
  let regimens = [];
  ...
  return {
    ...
    product:{
      ...
      useStatus:clean(row.pediatric_use_status),
      populationKey:clean(phase9Context?.populationKey),
      populationStatus:clean(phase9Context?.populationStatus),
      substanceConceptId:clean(phase9Context?.substanceConceptId,80) || null,
      substanceCanonicalName:clean(phase9Context?.substanceCanonicalName,240) || null,
      conceptKind:clean(phase9Context?.conceptKind,40) || null,
      clinicalVariantId:clean(phase9Context?.clinicalVariantId,80) || null,
      variantStatus:clean(phase9Context?.variantStatus,80) || 'UNAVAILABLE',
      productCount:Number.isInteger(Number(phase9Context?.productCount))
        ? Number(phase9Context.productCount) : null,
      phase9Context:phase9Context ? {
        contextVersion:clean(phase9Context.contextVersion,80),
        identityStatus:clean(phase9Context.identityStatus,80),
        formKey:clean(phase9Context.formKey,120),
        releaseKey:clean(phase9Context.releaseKey,120),
        routeKey:clean(phase9Context.routeKey,120),
        v3Published:phase9Context.v3Published === true,
        v3ProductKey:clean(phase9Context.v3ProductKey,180),
        v3VersionNo:Number(phase9Context.v3VersionNo) || null,
        source:phase9Context.source && typeof phase9Context.source === 'object'
          ? {
              sourceKey:clean(phase9Context.source.sourceKey,180),
              snapshotId:clean(phase9Context.source.snapshotId,80),
              sourceTier:clean(phase9Context.source.sourceTier,80),
              documentVersion:clean(phase9Context.source.documentVersion,180),
              documentDate:clean(phase9Context.source.documentDate,40),
            }
          : null,
      } : null,
...
async function calculatorCutover(rawSelector) {
  const selector = drugSelector(rawSelector);
...
function strictRuntimeUnavailable(scope) {
  return {
...
function mergeCanonicalContext(product, context) {
  if (!product || !context) return product;
  return {
    ...product,
    populationKey:clean(context.populationKey) || product.populationKey,
    populationStatus:clean(context.populationStatus) || product.populationStatus,
    substanceConceptId:clean(context.substanceConceptId) || product.substanceConceptId || null,
    substanceCanonicalName:clean(context.substanceCanonicalName) || product.substanceCanonicalName || product.substance || null,
    conceptKind:clean(context.conceptKind) || product.conceptKind || null,
    clinicalVariantId:clean(context.clinicalVariantId) || product.clinicalVariantId || null,
    variantStatus:clean(context.variantStatus) || product.variantStatus || 'UNAVAILABLE',
    productCount:Number.isInteger(Number(context.productCount)) ? Number(context.productCount) : (product.productCount ?? null),
  };
}
...
function calculatorTelemetryOutcome(result) {
  if (result?.error) return Number(result.status) === 404 ? 'NOT_FOUND' : 'BLOCKED';
  const outcome=clean(result?.calculation?.outcome).toUpperCase();
  if (['OUT_OF_RANGE','NOT_CALCULABLE','NEEDS_PATIENT_DATA'].includes(outcome)) return 'BLOCKED';
  return 'SERVED';
}

function recordCalculatorRuntime(runtimeSelector, decision, runtimeServed, result, startedAt, options = {}) {
  if (!runtimeSelector || !decision) return;
  void Cutover.recordEvent({
    selector:runtimeSelector,
    stateAvailable:decision.stateAvailable===true,
    controlVersion:decision.controlVersion,
    trafficBucketVersion:decision.trafficBucketVersion,
    mode:decision.mode,
    trafficBucket:decision.trafficBucket,
    selectedForV3:decision.selectedForV3===true,
    runtimeServed,
    v3Available:options.v3Available,
    fallbackUsed:options.fallbackUsed===true,
    outcome:calculatorTelemetryOutcome(result),
...
handler._test = Object.freeze({
  ...
  calculationBinding, patientFrom, readBody, loadPhase9Context, calculatorCutover,
  mergeCanonicalContext, calculatorTelemetryOutcome, recordCalculatorRuntime,
  loadLegacyProduct, calculateLegacyDose, FORBIDDEN_INPUT, MAX_BODY_BYTES,
});
