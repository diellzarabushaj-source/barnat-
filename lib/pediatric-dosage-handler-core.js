...
function searchFacets(results = []) {
  ...
  for (const item of results) {
    if (item.readiness === STATUS.CALCULATOR_READY) facets.ready += 1;
...
handler._test = Object.freeze({
  ...
  drugSelector, limitOf, searchPath, productPath, regimenPath, rankSearchRow,
