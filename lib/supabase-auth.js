...
async function loadOwnProfile(token, userId, options = {}) {
  const config = requireConfiguration();
  const select = 'id,full_name,avatar_url,specialty,license_number,role,status,legacy_user_id,verification_status,verification_submitted_at,verification_reviewed_at,created_at,updated_at';
  ...
  if (!['pending', 'active', 'suspended', 'disabled'].includes(status)) throw new SupabaseAuthError('PROFILE_STATUS_INVALID', 'MedIndex profile status is invalid.', 403);
  return {
    ...
    legacyUserId:String(profile.legacy_user_id || '').trim(),
    verificationStatus:String(profile.verification_status || 'missing').trim(),
    verificationSubmittedAt:String(profile.verification_submitted_at || '').trim(),
    verificationReviewedAt:String(profile.verification_reviewed_at || '').trim(),
    role,
    status,
  };
}
...
function assertActive(identity) {
  // A brand-new signup waits for an admin. It gets its own code so the browser can
  // say "waiting for approval" instead of a generic failure.
  if (identity?.status === 'pending') {
    throw new SupabaseAuthError(
      'ACCOUNT_PENDING_APPROVAL',
      'Llogaria jote është regjistruar dhe pret aprovimin e administratorit.',
      403,
    );
  }
...
function publicIdentity(identity) {
  ...
  return {
    ...
    profile:identity.profile ? {
      ...
      verificationStatus:identity.profile.verificationStatus,
    } : null,
  };
}
