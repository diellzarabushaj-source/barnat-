...
function evaluate({ rule = {}, patient = {}, context = {}, calculate } = {}) {
  ...
  const calculation = calculate({ rule, patient, context, renalAdjustment:renal.adjustment });
  if (!calculation || typeof calculation !== 'object' || typeof calculation.then === 'function') {
    return { outcome:OUTCOME.NOT_CALCULABLE, reasons:['Kalkulatori server-side nuk ktheu rezultat sinkron të vlefshëm.'] };
  }
