...
const Snapshot = require('./icd-hierarchy-snapshot.js');
...
const lastRefreshAttempts = new Map();

const clean = value => String(value ?? '').trim();
const SOURCE_URLS = Object.freeze([
  `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?tqx=out:csv&gid=${SHEET_GID}&range=A:J`,
  `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/export?format=csv&single=true&gid=${SHEET_GID}&range=A:J`,
]);
  ...
  for (const url of SOURCE_URLS) {
    ...
    try {
      try {
        ...
        failures.push(error?.name === 'AbortError'
          ? `timeout pas ${FETCH_TIMEOUT_MS}ms`
...
function enrichWithSnapshotMetadata(loaded) {
  if (!loaded?.data?.nodes?.length || loaded.sourceType === 'local-snapshot') return loaded;
  const metadata = FullIcd.nodeMap(Snapshot.load().data);
  for (const node of loaded.data.nodes) {
    const source = metadata.get(node.code);
    if (!source) continue;
    node.primaryCareRole = source.primaryCareRole || '';
    node.managementSummary = source.managementSummary || '';
    node.urgencyLevel = source.urgencyLevel || 'none';
    node.isUrgent = Boolean(source.isUrgent);
    node.isDirectUrgency = Boolean(source.isDirectUrgency);
    node.searchText = FullIcd.normalize([
      node.searchText, node.primaryCareRole, node.managementSummary,
    ].join(' '));
  }
  return loaded;
}
...
async function build(options = {}) {
  if (!options.sheetOnly) {
    try {
      ...
      if (neon) return enrichWithSnapshotMetadata(neon);
    } catch (error) {
...
async function load(options = {}) {
  ...
  if (!options.sheetOnly && !cached) {
    cached = Snapshot.load();
    caches.set(key, cached);
  }
  if (!force && cached && Date.now() - cached.loadedAt < CACHE_TTL_MS) return cached;
  if (!force && cached) {
    const lastAttempt = Number(lastRefreshAttempts.get(key) || 0);
    if (!pendingLoads.has(key) && Date.now() - lastAttempt >= REFRESH_RETRY_MS) {
      lastRefreshAttempts.set(key, Date.now());
...
function sourceMeta(loaded) {
  ...
  const snapshot = type === 'local-snapshot';
  return {
    type,
    status:loaded?.stale ? 'stale' : (neon ? 'active' : 'live'),
    visibility:neon ? 'private-mirror' : (snapshot ? 'bundled-cache' : 'public-link'),
...
module.exports = {
  ...
  sourceMeta,
  load,
  _test:{ revisionOf, resetForTests, buildSheet, build, fetchCsv, fetchSource, enrichWithSnapshotMetadata },
};
