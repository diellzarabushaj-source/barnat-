...
const MATCH_LABELS = Object.freeze({
  ...
  'title-la-exact':'Titull i saktë latinisht',
  'title-la-prefix':'Titull latinisht',
  'fuzzy-la':'Përputhje e afërt latinisht',
  'tokens-la':'Terma latinisht',
...
function profile(node) {
  ...
  const value = {
    ...
    laStems:unique(laTokens.map(stemToken)),
  };
...
function tokenMatch(queryToken, candidateToken, wildcard = false) {
  ...
  const longest = Math.max(queryStem.length, candidateStem.length);
  const maxDistance = longest >= 12 ? 3 : longest >= 7 ? 2 : longest >= 4 ? 1 : 0;
  if (!maxDistance) return null;
  const distance = boundedDistance(queryStem, candidateStem, maxDistance);
  if (distance <= maxDistance) {
    const lengthPenalty = Math.max(0, Math.abs(queryStem.length - candidateStem.length) * 2);
    return { score:58 - (distance * 7) - lengthPenalty, fuzzy:true };
  }
  return null;
}
...
function matchTextField(rawQuery, normalizedQuery, value, tokens, field) {
  ...
  const weights = {
    sq:{ exact:920, prefix:760, wildcard:630, tokens:430 },
    la:{ exact:890, prefix:730, wildcard:605, tokens:405 },
    en:{ exact:860, prefix:700, wildcard:580, tokens:370 },
  }[field] || { exact:840, prefix:680, wildcard:560, tokens:350 };
  ...
  if (value === normalizedQuery) {
    return { score:weights.exact, type:`title-${field}-exact`, field, matchedTerm:value };
  }
  if (value.startsWith(normalizedQuery)) {
    return { score:weights.prefix, type:`title-${field}-prefix`, field, matchedTerm:value };
  }
  if (hasWildcard) {
    const wildcardMatch = scoreTokens(queryTokens, tokens, wildcardTokens);
    if (wildcardMatch) return { score:weights.wildcard + wildcardMatch.score, type:'wildcard', field, matchedTerm:wildcardMatch.matched.join(' ') };
  }
  const tokenResult = scoreTokens(queryTokens, tokens, wildcardTokens);
  if (!tokenResult) return null;
  return {
    score:weights.tokens + tokenResult.score,
...
function bestMatch(node, rawQuery) {
  ...
  const candidates = [
    ...
    matchTextField(rawQuery, query, nodeProfile.la, nodeProfile.laTokens, 'la'),
...
function groupFor(item) {
  ...
  if (['code-exact', 'title-sq-exact', 'title-en-exact', 'title-la-exact'].includes(type)) return 'exact';
...
function suggestDataset(dataset, rawQuery, { limit = 18 } = {}) {
  ...
  const exact = ranked.find(item => ['code-exact', 'title-sq-exact', 'title-en-exact', 'title-la-exact'].includes(item.match.type));
  const selected = [];
  const seen = new Set();
