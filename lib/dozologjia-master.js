...
function limits(r) {
  const rows=[...r.maxima];
  if(r.Daily_Dose_Unit==='mg/kg/day'&&!['WEIGHT_DAILY','FIXED_DAILY'].includes(r.Dose_Basis))rows.push({Max_Type:'DAILY_WEIGHT',Max_Min:r.Daily_Dose_Min,Max_Unit:'mg/kg/day',Parse_Status:'PARSED',Display_Max:`${r.Daily_Dose_Min} mg/kg/ditë — kufiri më i ulët i intervalit të burimit`});
  else if(r.Daily_Dose_Unit==='mg/day'&&positive(r.Daily_Dose_Max))rows.push({Max_Type:'DAILY',Max_Min:r.Daily_Dose_Max,Max_Unit:'mg/day',Parse_Status:'PARSED',Display_Max:`${r.Daily_Dose_Max} mg/ditë`});
  return rows;
}
...
function catalog() {
  return {metadata:data.metadata,regimens:data.regimens.map(r=>({id:r.Atomic_Regimen_ID,drugId:r.Drug_ID,indicationId:r.Indication_ID,drug:r.drug.Canonical_Name,indication:r.indication.Indication_Name,route:r.Route_Code,population:r.Population_Display,frequency:r.Display_Frequency,duration:r.Display_Duration,safety:r.Safety_Text,unit:r.Dose_Unit,status:r.Release_Status,needs:requirements(r),gates:required(r).filter(g=>g.Gate_Type!=='PRODUCT_MATCH'),
    products:r.Product_Output_Mode==='DOSE_ONLY_NO_PRODUCT_CONVERSION'?[]:r.bindings.map(b=>({id:b.Formulation_ID,name:b.product.Product_Name,strength:b.product.Strength_Display,form:b.product.Form_Type})),steps:r.steps.map(s=>({id:s.Step_ID,order:s.Step_Order,label:s.Trigger}))}))};
}
...
function calculate(input) {
  ...
  if(n.age)for(const side of ['Min','Max'])if(!missing(r[`Age_${side}_Value`])){
    ...
    if(!compare(actual,r[`Age_${side}_Value`],r[`Age_${side}_Op`]))errors.push(`Mosha duhet të jetë ${r[`Age_${side}_Op`]} ${r[`Age_${side}_Value`]} ${r[`Age_${side}_Unit`]}.`);
  }
  if(n.weight&&!positive(input.weight))errors.push('Shëno peshën reale në kg.');
  for(const side of ['Min','Max'])if(!compare(input.weight,r[`Weight_${side}_Kg`],r[`Weight_${side}_Op`]))errors.push(`Pesha duhet të jetë ${r[`Weight_${side}_Op`]} ${r[`Weight_${side}_Kg`]} kg.`);
  ...
  for(const g of required(r))if(g.Gate_Type!=='PRODUCT_MATCH'&&input.gates?.[g.Gate_ID]!=='PASS')errors.push(g.Rule_Text);
  ...
  if(binding&&dose&&dose.period!=='day'&&r.Product_Output_Mode==='EXACT_PRODUCT_ALLOWED'&&!['LOCAL_APPLICATION','WHOLE_UNIT_ONLY','DISPLAY_AVAILABLE_STRENGTH','FIXED_VOLUME'].includes(binding.Conversion_Mode)){
    ...
    if(parenteral&&(r.Prep_Output_Mode!=='AUTO_PREP_ALLOWED'||!prep))notices.push('Mungon përgatitja e lidhur për këtë produkt dhe rrugë; mL nuk llogariten.');
      ...
      if(positive(conc)&&mass[dose.unit]&&['g/mL','mg/mL','mcg/mL'].includes(cu)&&(!parenteral||(['READY','READY_INFUSION'].includes(prep.Prep_Type)&&prep.Final_Concentration_Min===prep.Final_Concentration_Max))){
        ...
        if(parenteral){
          preparation={id:prep.Prep_ID,instruction:prep.Display_Preparation,notes:prep.Notes,timeMin:prep.Administration_Time_Min,timeMax:prep.Administration_Time_Max,timeUnit:prep.Administration_Time_Unit,source:prep.Source_ID};
  ...
  if(r.Product_Output_Mode==='DOSE_ONLY_NO_PRODUCT_CONVERSION'||(!binding&&dose?.unit!=='mL'))notices.push('Vetëm doza. Konvertimi kërkon formulim të lidhur në Master.');
  ...
  return {outcome:dose?'CALCULATED':'INSTRUCTION',errors:[],id:r.Atomic_Regimen_ID,drug:r.drug.Canonical_Name,indication:r.indication.Indication_Name,route:r.Route_Code,population:r.Population_Display,certification:data.metadata.certification,dose,conversion,preparation,step:step||null,
    instruction:basis==='LOCAL_APPLICATION'?r.Display_Dose:null,frequency:{type:r.Frequency_Type,min:r.Interval_Min,max:r.Interval_Max,unit:r.Interval_Unit,timesMin:r.Doses_Per_Day_Min,timesMax:r.Doses_Per_Day_Max},
    duration:{type:r.Duration_Type,min:r.Duration_Min,max:r.Duration_Max,unit:r.Duration_Unit},frequencyNote:r.Display_Frequency,durationNote:r.Display_Duration,safety:r.Safety_Text,maxima:limits(r),notices,sources:data.sources.filter(s=>refs.has(s.Source_ID)),product:binding?{name:binding.product.Product_Name,strength:binding.product.Strength_Display}:null};
}
