...
function catalog() {
  return {metadata:data.metadata,regimens:data.regimens.map(r=>({id:r.Atomic_Regimen_ID,drugId:r.Drug_ID,indicationId:r.Indication_ID,
    ...
    templates:shelf.TEMPLATES[r.Drug_ID]||[],caution:shelf.CAUTIONS[r.Drug_ID]||'',
...
function calculate(input) {
  ...
  if(r.Product_Output_Mode==='DOSE_ONLY_NO_PRODUCT_CONVERSION'||(!binding&&dose?.unit!=='mL'))notices.push('Master-i nuk lidh produkt me këtë skemë: sasia për të matur llogaritet nga produkti që zgjedh ti.');
