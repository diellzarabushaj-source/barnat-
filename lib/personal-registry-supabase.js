...
function lookupHints(keys) {
  ...
  keys.forEach(original => {
    ...
    if (!key) return;
    if (key.startsWith('drug:')) key = key.slice(5);
    ...
    const first = parts[0] || '';
    if (/^\d{6,}$/.test(first)) pdids.add(first);
    else if (/^\d+$/.test(first)) registryNumbers.add(first);

    if (/^\d{6,}$/.test(key)) pdids.add(key);
    else if (/^\d+$/.test(key)) registryNumbers.add(key);
  });
...
async function resolvePersonalDrugRows(request, mode) {
  ...
  if (!user?.id) throw new PersonalRegistryError(401, 'Kërkohet autentikim.', 'AUTH_REQUIRED');

  const keys = await membershipKeysForUser(user.id, normalizedMode);
