...
function ruleSourceValid(rule = {}) {
  ...
  return /^[0-9a-f]{64}$/i.test(snapshot)
    ...
    && Boolean(source.documentVersion || source.documentDate)
    && source.official === true
    && renal.every(adjustmentSourceValid)
    && hepatic.every(adjustmentSourceValid)
...
module.exports = {
  ...
  rpcStrict,
  isRpcMissing,
  _test:{ clean, enabledFlag, bodyFor, unwrap, adjustmentSourceValid, ruleSourceValid, payloadValid },
};
