...
const ALLOWED_ENTITY_TYPES = Object.freeze(['drug', 'drug-note', 'lab', 'icd', 'protocol']);
...
function validateFavoritePayload(entityType, value) {
  const payload = plainObject(value) ? value : {};
  if (entityType !== 'drug-note') return payload;
...
function normalizedFavorite(item) {
  ...
  return {
    entityType,
    entityKey,
    payload:validateFavoritePayload(entityType, item.payload),
