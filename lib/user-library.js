...
async function getPersonalSummary(user, authUid = '') {
  ...
  const [favorites, notes] = await Promise.all([
    countPrivateRows('user_favorites', storageUid, { entity_type:'eq.product' }),
...
function personalProductPayload(row) {
  ...
  return {
    ...
    registryNumber:Number.isSafeInteger(Number(row?.registry_number)) ? Number(row.registry_number) : null,
...
async function getSnapshot(user, authUid = '') {
  ...
  try {
    const productMetadata=await personalProductMetadataByIds(productIds);
    legacyFavorites=legacyFavorites.map(item => mergePersonalProductMetadata(item,productMetadata));
