...
async function personalProductMetadataByIds(ids) {
...
async function personalProductMetadataByPdids(pdids) {
...
async function getSnapshot(user, authUid = '') {
  ...
  let productMetadataWarning='';
  const productIds=[
    ...legacyFavorites.filter(row => row.entityType === 'product').map(row => row.entityKey),
    ...entityNotes.filter(row => row.entityType === 'product').map(row => row.entityKey),
  ];
  const legacyPdids=legacyFavorites.filter(row => row.entityType === 'drug').map(legacyFavoritePdid).filter(Boolean);
  try {
    const [productMetadata,legacyMetadata]=await Promise.all([
      personalProductMetadataByIds(productIds),
      personalProductMetadataByPdids(legacyPdids),
    ]);
    legacyFavorites=legacyFavorites
      .map(item => mergePersonalProductMetadata(item,productMetadata))
      .map(item => mergeLegacyFavoriteMetadata(item,legacyMetadata));
    entityNotes=entityNotes.map(item => mergePersonalProductMetadata(item,productMetadata));
  } catch (error) {
    productMetadataWarning=clean(error?.message,240);
    console.warn('Personal product metadata hydration failed:', productMetadataWarning);
  }
  ...
  return {
    ...
    warnings:{
      ...(decryptionErrors.length ? { unreadablePrescriptions:decryptionErrors.length } : {}),
      ...(productMetadataWarning ? { personalProductMetadata:productMetadataWarning } : {}),
    },
    generatedAt:nowIso(),
  };
}
