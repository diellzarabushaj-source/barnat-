...
async function libraryRows(userId) {
  const [prescriptions, favorites] = await Promise.all([
  ...
  return { prescriptions, favorites };
}
...
function normalizedFavorite(item) {
  ...
  if (!['drug', 'lab', 'icd', 'protocol'].includes(entityType) || !entityKey) {
    throw new HttpError(400, 'Një favorit ka identifikues të pavlefshëm.');
  }
  return {
    entityType,
    entityKey,
    payload:plainObject(item.payload) ? item.payload : {},
...
function normalizedFavoriteTombstone(item) {
  const entityType = clean(item?.entityType || 'drug', 40);
  const entityKey = clean(item?.entityKey, 300);
  if (!['drug', 'lab', 'icd', 'protocol'].includes(entityType) || !entityKey) return null;
...
async function syncSnapshot(user, body) {
  ...
  if (favorites.length > MAX_FAVORITES) throw new HttpError(413, 'Numri i favoriteve e tejkalon kufirin e bibliotekës.');
  ...
  return { prescriptions:prescriptionRecords.length, favorites:favoriteRecords.length };
}
