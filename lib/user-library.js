...
async function getSnapshot(user) {
  ...
  return {
    ok:true,
    version:1,
    // The account this snapshot belongs to. The client stamps it on the local
    // library so a second account signing in on the same device can never merge
    // the first one's favourites and notes into its own.
