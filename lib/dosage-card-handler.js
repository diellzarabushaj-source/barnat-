...
async function handleSingle(req,res,startedAt) {
  ...
  const [regimenResult,profileResult]=await Promise.allSettled([
    readRows(regimenPath(drugId),'Supabase clinical card dosage'),
    readRows(profilePath(drugId),'Supabase clinical card profile'),
  ]);
  if(regimenResult.status!=='fulfilled') throw regimenResult.reason;
  const regimens=regimenResult.value;
  ...
  return res.status(200).json({
    ok:true,drugId,adult,pediatric,profile,sources,
    meta:{
      dataSource:'supabase',
