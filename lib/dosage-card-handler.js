...
function isBatchRequest(req) {
  return requestUrl(req).searchParams.get('view') === 'cards';
}
...
function sourceList(adult, pediatric, profile = { sourceUrls:[] }) {
  return [...new Set([
    adult?.sourceUrl,
    pediatric?.sourceUrl,
    ...(profile.sourceUrls || []),
...
function publicBatchCard(drug, rows) {
  ...
  return {
    registryNumber:clean(drug.registry_number),
    ...
    adultDose:clean(adult?.dose),
    adultRoute:clean(adult?.route),
    pediatricDose:clean(pediatric?.dose),
    pediatricRoute:clean(pediatric?.route),
    sourceUrls:sourceList(adult, pediatric),
  };
}
...
async function handleBatch(req, res, startedAt) {
  ...
  if (numbers === null || !numbers.length) {
    ...
    return res.status(400).json({ error:`Jep 1–${MAX_BATCH_DRUGS} numra validë të regjistrit.` });
  }

  const drugs = await readRows(drugsByRegistryPath(numbers), 'Visible registry dosage drugs');
  const ids = drugs.map(row => clean(row.id)).filter(UUID_RE.test.bind(UUID_RE));
  const regimens = ids.length ? await readRows(batchRegimenPath(ids), 'Visible registry dosage cards') : [];
  const byDrug = new Map();
  for (const row of regimens) {
    const drugId = clean(row.drug_id);
    if (!byDrug.has(drugId)) byDrug.set(drugId, []);
    byDrug.get(drugId).push(row);
  }
  const cards = drugs.map(drug => publicBatchCard(drug, byDrug.get(clean(drug.id)) || []));

  setCommonHeaders(res, startedAt, 'dosagecards');
  if (req.method === 'HEAD') return res.status(200).end();
  return res.status(200).json({
    ok:true,
    cards,
    meta:{
      dataSource:'neon', targeted:true, batch:true,
      requested:numbers.length, drugs:drugs.length, regimenRows:regimens.length,
    },
  });
}
...
async function handleSingle(req, res, startedAt) {
  ...
  setCommonHeaders(res, startedAt, 'clinicalcard');
  if (req.method === 'HEAD') return res.status(200).end();
  return res.status(200).json({
    ok:true,
    drugId,
...
async function handler(req, res) {
  ...
  try {
    ...
    console.error(isBatchRequest(req) ? 'Visible registry dosage cards error:' : 'Targeted clinical card error:', error);
    res.setHeader('Cache-Control', 'no-store');
...
handler._test = Object.freeze({
  requestDrugId, requestRegistryNumbers, isBatchRequest, regimenPath, profilePath,
  drugsByRegistryPath, batchRegimenPath, chooseRegimen, publicRegimen, publicProfile,
  sourceList, publicBatchCard,
});
