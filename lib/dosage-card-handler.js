...
function productPath(drugId) {
  const params = new URLSearchParams();
  params.set('select', [
    'id','registry_number','pdid','trade_name','active_substance','strength','pharmaceutical_form',
    'pediatric_dose_summary','pediatric_route','pediatric_source_url','pediatric_verification_status','source_payload'
  ].join(','));
  params.set('id', `eq.${drugId}`);
  params.set('limit', '1');
  return `drugs?${params.toString()}`;
}
function drugsByRegistryPath(numbers) {
  const params = new URLSearchParams();
  params.set('select', 'id,registry_number,pdid,trade_name,strength,drug_class,use_text,pediatric_dose_summary,pediatric_route,pediatric_source_url,pediatric_verification_status,source_payload');
...
function sheetValue(product, ...labels) {
  const source=product?.source_payload && typeof product.source_payload==='object' && !Array.isArray(product.source_payload)
    ? product.source_payload
    : {};
  for (const label of labels) {
    const value=clean(source[label]);
    if (value) return value;
  }
  return '';
}
function productFallback(product, population) {
  if (!product) return null;
  if (population === 'pediatric') {
    const dose=clean(product.pediatric_dose_summary)
      || sheetValue(product,'Doza pediatrike — përmbledhje','Doza e plotë — Fëmijë','Doza — Fëmijë','Doza pediatrike');
    const route=clean(product.pediatric_route) || sheetValue(product,'Rruga pediatrike','Rruga — Fëmijë');
    const sourceUrl=safeUrl(product.pediatric_source_url || sheetValue(product,'Burimi pediatrik'));
    const maximum=sheetValue(product,'Maks. në 24h — vlerë');
    if (!dose && !route && !maximum) return null;
    return {
      ...
      duration:'',
      maximum,
      warnings:sheetValue(product,'Kufizim / mos-përdorim pediatrik'),
      indication:sheetValue(product,'Indikacioni pediatrik'),
      sourceUrl,
      reviewedAt:null,
      verification:clean(product.pediatric_verification_status) || 'sheet',
      provenance:'sheet',
    };
  }
  const dose=sheetValue(product,'Doza e plotë — Të rritur','Doza — Të rritur','Doza të rritur','Doza e të rriturve');
  const route=sheetValue(product,'Rruga — Të rritur','Rruga e të rriturve');
  const maximum=sheetValue(product,'Maks. në 24h — vlerë');
  if (!dose && !route && !maximum) return null;
  return {
    population:'adult',
...
function mergeRegimen(primary, fallback) {
  ...
  return {
    ...
    frequency:clean(primary.frequency) || clean(fallback.frequency),
    duration:clean(primary.duration) || clean(fallback.duration),
    maximum:clean(primary.maximum) || clean(fallback.maximum),
    warnings:clean(primary.warnings) || clean(fallback.warnings),
    indication:clean(primary.indication) || clean(fallback.indication),
    sourceUrl:safeUrl(primary.sourceUrl) || safeUrl(fallback.sourceUrl),
    provenance:clean(primary.dose) ? 'dosage_regimens' : clean(fallback.provenance),
  };
}
function publicRegimens(rows, population) {
  return (Array.isArray(rows) ? rows : [])
    .filter(row => clean(row.population) === population)
    .map(publicRegimen)
    .filter(Boolean);
}
...
function publicBatchCard(drug, rows) {
  const adult = mergeRegimen(chooseRegimen(rows, 'adult'), productFallback(drug, 'adult'));
  const pediatric = mergeRegimen(chooseRegimen(rows, 'pediatric'), productFallback(drug, 'pediatric'));
...
async function handleSingle(req,res,startedAt) {
  ...
  const [regimenResult,profileResult,productResult]=await Promise.allSettled([
    readRows(regimenPath(drugId),'Supabase clinical card dosage'),
    readRows(profilePath(drugId),'Supabase clinical card profile'),
    readRows(productPath(drugId),'Supabase clinical card product fallback'),
  ]);
  const regimens=regimenResult.status==='fulfilled' ? regimenResult.value : [];
  const profiles=profileResult.status==='fulfilled' ? profileResult.value : [];
  const product=productResult.status==='fulfilled' ? productResult.value[0] : null;
  if(profileResult.status!=='fulfilled') console.warn('Clinical profile unavailable; dosage card will continue without profile:', profileResult.reason?.message || profileResult.reason);
  if(productResult.status!=='fulfilled') console.warn('Product sheet fallback unavailable:', productResult.reason?.message || productResult.reason);

  const adultRegimens=publicRegimens(regimens,'adult');
  const pediatricRegimens=publicRegimens(regimens,'pediatric');
  const adult=mergeRegimen(chooseRegimen(regimens,'adult'),productFallback(product,'adult'));
  const pediatric=mergeRegimen(chooseRegimen(regimens,'pediatric'),productFallback(product,'pediatric'));

  if(regimenResult.status!=='fulfilled' && !adult?.dose && !pediatric?.dose) throw regimenResult.reason;
  const profile=publicProfile(profiles[0]), sources=sourceList(adult,pediatric,profile);
  setCommonHeaders(res,startedAt,'supabase-clinicalcard');
  if(req.method==='HEAD') return res.status(200).end();
  return res.status(200).json({
    ok:true,drugId,adult,pediatric,adultRegimens,pediatricRegimens,profile,sources,
    meta:{
      dataSource:'supabase',
      regimenRows:regimens.length,
      targeted:true,
      profileAvailable:profileResult.status==='fulfilled',
      productFallbackAvailable:Boolean(product),
      regimenAvailable:regimenResult.status==='fulfilled',
    },
  });
}
...
handler._test=Object.freeze({requestDrugId,requestRegistryNumbers,isBatchRequest,regimenPath,profilePath,productPath,drugsByRegistryPath,batchRegimenPath,chooseRegimen,publicRegimen,publicRegimens,productFallback,mergeRegimen,publicProfile,sourceList,publicBatchCard});
