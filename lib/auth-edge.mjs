...
function validSessionPayload(data, now = Date.now()) {
  ...
  // Mirrors SUPABASE_PROVIDERS in lib/auth.mjs. The edge runtime cannot share that
  // module, so the two lists are kept identical by hand — a provider added there
  // and missed here would be rejected at the edge and accepted in the function.
  if (!['supabase-google', 'supabase-password', 'legacy-password', 'legacy-session'].includes(provider)) return false;
  if (provider === 'supabase-google' || provider === 'supabase-password') {
