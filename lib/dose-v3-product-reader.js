...
async function build(selector){
 ...
 return {schemaVersion:'dose-product-fast-path-v3',
  product:{productKey:clean(bindings[0].product_key),drugId:clean(bindings[0].drug_id),rules:publicRules},
  meta:{dataSource:'supabase-v3',failClosed:true,publishedOnly:true,officialVerifiedOnly:true,
    rules:publicRules.length,dbReads:3,runtimeModel:'v3'}};
}
module.exports={build,_test:{pathFor,inIds,clean}};
