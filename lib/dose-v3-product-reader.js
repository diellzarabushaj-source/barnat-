...
async function buildMultiRead(selector) {
  ...
  const renalAdjustments = await adjustmentRows(
    ...
    [
      ...
      'source_document_version','source_document_date','review_status'
    ].join(','),
  ...
  const hepaticAdjustments = await adjustmentRows(
    ...
    [
      ...
      'replacement_dose_min','replacement_dose_max','interval_min_hours','interval_max_hours',
      'source_key','source_snapshot_id','source_section','source_section_sha256','source_evidence_hash',
      'source_document_version','source_document_date','review_status'
    ].join(','),
  ...
  for (const row of renalAdjustments.filter(sourceValid)) {
  ...
  for (const row of hepaticAdjustments.filter(sourceValid)) {
    const key = clean(row.rule_id);
...
module.exports = {
  build,
  buildMultiRead,
  _test:{ pathFor, inIds, clean, sourceValid, productSourceValid, adjustmentRows },
};
