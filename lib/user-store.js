...
async function userFromSession(request) {
  ...
  // A Supabase session only decodes when Supabase Auth already verified an
  ...
  const sessionRole = ['supabase-google', 'supabase-password'].includes(session.provider) ? clean(session.authRole) : '';
  ...
  if (!row && session.email && sessionAuthorized) {
    try {
      ...
      return UserIdentity.attachSessionIdentity(created, session);
    } catch {
      return null;
    }
  }
  ...
  return UserIdentity.attachSessionIdentity(user, session);
}
