...
function normalizeRule(input = {}) {
  ...
  const rule = {
    ...
    sourceSectionSha256:clean(input.sourceSectionSha256 || input.source_section_sha256),
...
function validateRule(input = {}) {
  ...
  if (publicationLike) {
    ...
    if (!/^[0-9a-f]{64}$/i.test(rule.sourceSectionSha256)) errors.push('source_section_sha256_missing_or_invalid');
    if (!/^[0-9a-f]{64}$/i.test(rule.sourceEvidenceHash)) errors.push('source_evidence_hash_missing_or_invalid');
    if (
      /^[0-9a-f]{64}$/i.test(rule.sourceSnapshotId)
      && /^[0-9a-f]{64}$/i.test(rule.sourceEvidenceHash)
      && rule.sourceSnapshotId.toLowerCase() !== rule.sourceEvidenceHash.toLowerCase()
    ) {
