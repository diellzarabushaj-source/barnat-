...
function normalizeState(value){
  ...
  if(stateVersion==='drx-phase10-cutover-state-v1'){
    if(mode==='SHADOW' && percent===0 && !strictArmed && !strictSupported){
      return Object.freeze({
        ...value,
        stateVersion:'drx-phase10-cutover-state-v2',
        mode,
  ...
  // Phase 10G explicitly refuses strict activation even if a stale/misconfigured
  // environment or DB row attempts to expose it.
  if(mode==='STRICT' || strictArmed || strictSupported) return safeState('strict_not_supported_in_phase10g');
