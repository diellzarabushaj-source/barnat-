...
function normalizeState(value){
  if(!value || value.stateVersion!=='drx-phase10-cutover-state-v1') return safeState('invalid_version');
  ...
  const strictSupported=value.strictActivationSupported===true;
  if(!Number.isInteger(version) || version<1) return safeState('invalid_control_version');
  if(mode==='SHADOW' && percent===0 && !strictArmed){
    return Object.freeze({...value,mode,controlledPercent:0,controlVersion:version,strictArmed:false,stateAvailable:true});
  }
  if(mode==='CONTROLLED' && [1,5,10].includes(percent) && !strictArmed){
    return Object.freeze({...value,mode,controlledPercent:percent,controlVersion:version,strictArmed:false,stateAvailable:true});
  }
  // Phase 10B explicitly refuses strict activation even if a stale/misconfigured
  // environment or DB row attempts to expose it.
  if(mode==='STRICT' || strictArmed || strictSupported) return safeState('strict_not_supported_in_phase10b');
...
function decision(state,selector={}){
  ...
  const selected=normalized.stateAvailable===true
...
module.exports={
  ...
  _test:{clean,safeState,normalizeState,trafficBucket,clearCache,CACHE_MS,ERROR_CACHE_MS},
};
