...
async function saveDecision(payload) {
  ...
  if (input.decision === 'auto') {
    ...
    const row = {
      ...
      reviewed_by:'Dr. Diellza Rabushaj', reviewed_at:timestamp, source_key:sourceKey,
  ...
  await neonRequest('audit_logs', {
    ...
    body:[{
      ...
      changed_by:'Dr. Diellza Rabushaj', source:'population_verification', changed_at:timestamp,
    }],
    prefer:'return=minimal',
  });
...
async function handle(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      return res.status(200).json({ ok:true, item:await saveDecision(parseBody(req)) });
    }
...
module.exports = {
  ...
  _test:{ affirmativeRow, negativeRow, populationDecision, normalizeDecisionPayload, parseRegistryNumbers, sameOrigin },
};
