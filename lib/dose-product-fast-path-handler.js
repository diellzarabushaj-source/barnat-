...
async function buildRuntimePayload(selector) {
  if (!V3Gate.v3ReadsEnabled()) return { payload:await buildProductPayload(selector), runtime:'v2' };

  let v3Payload = null;
  let v3Error = null;
  try {
    v3Payload = await V3Reader.build(selector);
    if (v3Payload && !V3Gate.validateV3Payload(v3Payload)) {
      v3Error = new Error('DRx V3 payload failed contract validation.');
      v3Payload = null;
    }
  } catch (error) {
    v3Error = error;
  ...
  const decision = V3Gate.chooseRuntime({
    v3Enabled:true,
    v3Available:Boolean(v3Payload),
    v2Available:!V3Gate.v3StrictEnabled(),
    strictV3:V3Gate.v3StrictEnabled(),
  });

  if (decision.runtime === 'v3') return { payload:v3Payload, runtime:'v3' };
  if (decision.failClosed) {
    const error = v3Error || new Error(decision.reason || 'DRx V3 unavailable');
    error.code = 'DRX_V3_REQUIRED_UNAVAILABLE';
    throw error;
  }

  return { payload:await buildProductPayload(selector), runtime:'v2-fallback', v3Error };
}
...
async function handler(req, res) {
  try {
    ...
    if (!payload) return res.status(404).json({ error:'Produkti i publikuar nuk u gjet.' });
