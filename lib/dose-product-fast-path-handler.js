...
async function buildRuntimePayload(selector) {
  const readsEnabled = V3Gate.v3ReadsEnabled();
  const shadowEnabled = Shadow.shadowEnabled();

  if (!readsEnabled && !shadowEnabled) {
    return { payload:await buildProductPayload(selector), runtime:'v2' };
  }
  ...
  const v3Result = await readValidatedV3(selector);
  ...
  if (decision.runtime === 'v3') return { payload:v3Result.payload, runtime:'v3' };
  if (decision.failClosed) {
    const error = v3Result.error || new Error(decision.reason || 'DRx V3 unavailable');
    error.code = 'DRX_V3_REQUIRED_UNAVAILABLE';
    throw error;
  }

  return { payload:await buildProductPayload(selector), runtime:'v2-fallback', v3Error:v3Result.error };
}
