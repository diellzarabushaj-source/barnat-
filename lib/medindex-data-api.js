...
// Canonical MedIndex Data API transport.
// Runtime database traffic is Supabase-only; legacy Neon naming is compatibility-only.
...
const BULK_READ_TABLES = new Set([
  ...
  'drive_sheet_rows',
  'audit_logs',
  'medindex_system_health_snapshot_v1',
]);
...
const TRANSIENT_READ_STATUSES = new Set([408,425,429,500,502,503,504,520,521,522,523,524]);

function isRetryableSupabaseReadError(error, method = 'GET') {
  if (String(method || 'GET').toUpperCase() !== 'GET') return false;
  const status = Number(error?.status || 0);
  if (TRANSIENT_READ_STATUSES.has(status)) return true;
  const text = String(error?.message || '');
  return error?.name === 'TypeError'
    || /fetch failed|ECONNRESET|ETIMEDOUT|EAI_AGAIN|socket hang up/i.test(text);
}

function retryDelayMs(error, attempt = 1) {
  const providerSeconds = Number(error?.payload?.retry_after);
  if (Number.isFinite(providerSeconds) && providerSeconds > 0) {
    return Math.min(750, Math.max(100, Math.round(providerSeconds * 1000)));
  }
  return Math.min(750, Math.max(100, Number(attempt || 1) * 250));
}
  ...
  try {
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (attempt >= maxAttempts || !isRetryableSupabaseReadError(error, method)) throw error;
        await sleep(retryDelayMs(error, attempt));
      }
    }
    throw new Error('Supabase Data API request exhausted retries.');
  } finally {
