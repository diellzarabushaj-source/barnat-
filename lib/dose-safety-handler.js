...
async function buildCatalog() {
  ...
  const [sources, products, safetyRows] = await Promise.all([
  ...
  const catalog = products
    ...
    .map(product => {
      ...
      return safety.length ? { productKey:clean(product.product_key), safety } : null;
    })
    .filter(Boolean)
    .sort((a, b) => a.productKey.localeCompare(b.productKey));

  const payload = {
    schemaVersion:'dose-safety-v1.0.0',
...
handler._test = Object.freeze({ clean, lower, statusAllowed, validHttps, routeMatches, rowMatchesProduct, dedupeSafety, safetyPublic });
