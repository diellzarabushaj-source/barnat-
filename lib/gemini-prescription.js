...
function buildTargets(baseline, generateMissingSignatures, selectedDrugs = []) {
  ...
  baseline.sections.forEach((section, sectionIndex) => {
    ...
    section.medications.forEach((item, medicationIndex) => {
      ...
      if (item.individualSignature && manuallyWritten) return;
...
Diagnoza dhe identiteti i barit janë vetëm kontekst për ekranin; NUK lejohen si burim për dozë, rrugë, shpeshtësi, kohëzgjatje ose sasi.
