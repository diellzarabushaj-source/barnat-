...
function rowToNode(row, headers, rowNumber) {
  ...
  return {
    ...
    searchText:normalize([
      code, englishTitle, albanianDraft, record.Kapitulli, record.Blloku, record['Kodi prind'],
...
function queryDataset(dataset, params = {}) {
  ...
  let candidates = dataset?.nodes || [];
  if (parent) candidates = indexes.childrenByParent.get(parent) || [];
  else if (chapter) candidates = indexes.byChapter.get(chapter) || [];
  else if (levels.size) {
    candidates = [...levels].flatMap(requestedLevel => indexes.byLevel.get(requestedLevel) || []);
  }
