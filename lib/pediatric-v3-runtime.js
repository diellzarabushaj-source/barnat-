...
async function hydrateClinicalMetadata(payload) {
  ...
  const snapshots = [...new Set(rules.flatMap(rule => [
    rule?.source?.snapshotId,
    ...(rule.renalAdjustments || []).map(item => item?.source?.snapshotId),
    ...(rule.hepaticAdjustments || []).map(item => item?.source?.snapshotId),
  ]).map(clean).filter(Boolean))];

  const renalIds = rules.flatMap(rule => (rule.renalAdjustments || []).map(item => clean(item.adjustmentId))).filter(Boolean);
  const hepaticIds = rules.flatMap(rule => (rule.hepaticAdjustments || []).map(item => clean(item.adjustmentId))).filter(Boolean);

  const [sourceRows, renalRows, hepaticRows] = await Promise.all([
    snapshots.length
      ? readRows(
        'dose_source_snapshots_v3?select=snapshot_id,source_key,source_url,source_tier,document_version,document_date&snapshot_id='
          + encodeURIComponent(inFilter(snapshots)) + '&limit=200',
        'V3 calculator source hydration'
      )
      : [],
    renalIds.length
      ? readRows(
        'dose_renal_adjustments_v3?select=adjustment_id,review_status,verified_by,verified_at&adjustment_id='
          + encodeURIComponent(inFilter(renalIds)) + '&review_status=eq.verified&limit=200',
        'V3 calculator renal verification hydration'
      )
      : [],
    hepaticIds.length
      ? readRows(
        'dose_hepatic_adjustments_v3?select=adjustment_id,review_status,verified_by,verified_at&adjustment_id='
          + encodeURIComponent(inFilter(hepaticIds)) + '&review_status=eq.verified&limit=200',
        'V3 calculator hepatic verification hydration'
      )
      : [],
  ]);

  const sourceMap = new Map(sourceRows.map(row => [clean(row.snapshot_id), row]));
  const renalMap = new Map(renalRows.map(row => [clean(row.adjustment_id), row]));
  const hepaticMap = new Map(hepaticRows.map(row => [clean(row.adjustment_id), row]));
  ...
  function hydrateAdjustment(item, map) {
    ...
    if (!sourceRow || !/^https:\/\//i.test(clean(sourceRow.source_url))) return null;
    return {
      ...item,
      reviewStatus:clean(verification.review_status),
      verifiedBy:clean(verification.verified_by),
      verifiedAt:clean(verification.verified_at),
      source:{ ...item.source, url:clean(sourceRow.source_url), sourceTier:clean(sourceRow.source_tier) },
    };
  }
  ...
  for (const rule of rules) {
    ...
    if (!sourceRow || !/^https:\/\//i.test(clean(sourceRow.source_url))) continue;
    ...
    hydratedRules.push({
      ...
      source:{ ...rule.source, url:clean(sourceRow.source_url), sourceTier:clean(sourceRow.source_tier) },
    });
  }
  ...
  return {
    ...
    meta:{ ...(payload.meta || {}), calculatorHydrated:true },
  };
}
...
function demographicsState(rule, patient) {
  const missing = [];
  if (Core.requiredInputs(rule).includes('age_months') && patient.ageMonths === null) missing.push('age');
  if (Core.requiredInputs(rule).includes('weight_kg') && patient.weightKg === null) missing.push('weightKg');
  if (Core.requiredInputs(rule).includes('height_cm') && patient.heightCm === null) missing.push('heightCm');
  if (missing.length) return { match:false,missing };

  const age=patient.ageMonths, weight=patient.weightKg;
...
module.exports = {
  ...
  readPayload,
  _test:{
    clean,finite,selectorOf,pediatricRule,ageMonthsFromBody,patientFromBody,
    inFilter,rulesByIndication,advancedMeasures,requiresOf,scheduleSummary,
