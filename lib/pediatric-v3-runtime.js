...
function requiresOf(rules = []) {
  ...
  return {
    ...
    age:required.has('age_months') || required.has('age_days'),
...
function optionFromRules(indicationId, rules) {
  ...
  const clinicalVariants = [...new Map(
    rules
      .filter(rule => rule.conditionReviewRequired === true && clean(rule.regimenOptionKey))
      .map(rule => [
        clean(rule.regimenOptionKey),
        {
          key:clean(rule.regimenOptionKey),
          label:clean(rule.conditionText) || clean(rule.regimenOptionKey),
        },
      ])
  ).values()];
  const treatmentDays = rules
    .flatMap(rule => [finite(rule.startDay), finite(rule.endDay)])
  ...
  return {
    ...
    clinicalVariants,
    treatmentDayRange:treatmentDays.length
      ? { min:Math.min(...treatmentDays), max:Math.max(...treatmentDays) }
      : null,
    regimen:{
      indication:clean(first.indicationName),
      route:routes.length === 1 ? routes[0] : '',
