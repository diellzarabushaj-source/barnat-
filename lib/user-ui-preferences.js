...
// Canonical registry column ids. This list is the persistence contract for the
// table: anything the picker in registry-v2.js can tick must be storable here,
// otherwise the save round-trip silently drops the column and the checkmark
// disappears on the next refresh. tests/registry-user-preferences-test.js keeps
// both sides in sync.
const COLUMN_IDS = Object.freeze([
  'registry','name','substance','strength','form','prescription',
  'drugClass','use','population','atc',
...
// "Nothing stored yet" and "the user ticked exactly these" are different
// answers. Only the first one falls back to the default table; an explicit
// selection is honoured as sent, minus ids this build does not know.
function normalizeColumns(value) {
  if (!Array.isArray(value)) return [...DEFAULT_COLUMNS];
  const requested = value.map(clean).filter(Boolean);
