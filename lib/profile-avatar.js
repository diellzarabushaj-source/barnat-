...
async function profileRow(authUid) {
  const { data } = await supabaseRequest(
    ...
    { privileged:true, timeoutMs:5000, label:'Profile avatar lookup' },
  );
...
async function updateProfileAvatar(authUid, value) {
  await supabaseRequest(`profiles?id=eq.${encodeURIComponent(authUid)}`, {
    privileged:true,
    method:'PATCH',
...
async function route(req, res) {
  try { return await handle(req, res); }
    ...
    if (status >= 500 || !(error instanceof ProfileAvatarError)) {
