...
async function profileRows() {
  const { data } = await neonRequest(
    'profiles?select=id,full_name,specialty,license_number,role,status,legacy_user_id,verification_status,verification_submitted_at,verification_reviewed_at,created_at,updated_at'
...
async function listUsers(options = {}) {
  const [profiles, accounts, documentsResult] = await Promise.all([
    ...
    neonRequest(
      'verification_documents?select=id,user_id,original_filename,mime_type,byte_size,status,rejection_reason,reviewed_at,created_at'
      + '&order=created_at.desc&limit=500',
    ),
  ]);
...
module.exports = {
  ...
  listUsers,
  updateUser,
  _test:{ supabaseUrl, secretKey, authUsers, mergeUser, storageUidOf, assertSafeChange, activeAdminCount, verifyCsrf, mappedReviewError },
};
