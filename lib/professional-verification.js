...
async function pendingProfile(userId) {
  const { data } = await neonRequest(
    `profiles?select=id,status,verification_status,verification_submitted_at&id=eq.${encodeURIComponent(userId)}&limit=1`,
  );
...
async function uploadVerification(req, options = {}) {
  ...
  const document = decodeDocument(await readJsonBody(req));
  ...
  try {
    ...
    return {
      ...
      status:'submitted',
      message:'Dokumenti u ruajt privatisht dhe regjistrimi pret shqyrtimin e administratorit.',
    };
  } catch (error) {
    await deleteObject(storagePath, fetchImpl);
    if (/verification_documents_one_uploaded|duplicate key/i.test(String(error?.message || ''))) {
...
async function enrollmentStatus(req) {
  ...
  const { data } = await neonRequest(
    `verification_documents?select=id,status,original_filename,mime_type,byte_size,created_at&user_id=eq.${encodeURIComponent(identity.authUid)}&order=created_at.desc&limit=1`,
  );
