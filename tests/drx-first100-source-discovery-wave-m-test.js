...
const wave = read('data/drx-first100-source-discovery-wave-m-v1.json');
...
const prior = [...'abcdefghijkl'].flatMap(letter => read('data/drx-first100-source-discovery-wave-' + letter + '-v1.json').rows);
...
const faktu = wave.rows[0];
assert.equal(faktu.canonicalKey, 'cinchocainepolicresulen');
assert.equal(faktu.sourceTier, 'EU_NATIONAL');
assert.equal(faktu.registrationNumber, '23/0126/87-CS');
...
console.log('DRx first-100 official source discovery wave M contract passed.');
