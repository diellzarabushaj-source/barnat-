...
assert.equal(guidance.VERSION, 'icd-clinical-guidance-v1');
...
assert.match(recovery, /const API_PATH = '\/api\/icd'/);
assert.match(recovery, /root\.fetch\(API_PATH/);
assert.match(recovery, /api\.buildIndex\(entries\)/);
assert.match(recovery, /api\.resolveClinicalContext\(code, index\)/);
assert.match(recovery, /renderRecovered\(context, api/);
assert.doesNotMatch(recovery, /internalRetry\.click|stopImmediatePropagation|location\.reload|localStorage|sessionStorage/);
assert.match(css, /@media \(max-width: 520px\)/);
assert.match(css, /@media \(forced-colors: active\)/);
assert.match(css, /overflow-wrap: anywhere/);

console.log('ICD clinical guidance, curated Google Sheet provenance, deterministic in-place recovery and non-fabrication contract passed.');
