...
test('ICD workspace source health exposes the live revision and preserves cached context offline', async ({ page, context }) => {
  await page.setViewportSize({ width:390, height:844 });
  await installAdvancedRoute(page);
  await openTree(page);
  ...
  await page.screenshot({ path:path.join(OUTPUT, 'icd-workspace-source-offline-mobile.png'), fullPage:true });
