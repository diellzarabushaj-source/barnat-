...
const MIGRATION = fs.readFileSync(
  path.join(ROOT, 'supabase', 'migrations', '20260827142058_p1_verified_exact_identity_batch_five.sql'),
  'utf8'
);
...
assert.ok(
  manifest.migrations.some(item =>
    String(item.version) === '20260827142058' &&
    item.name === 'p1_verified_exact_identity_batch_five'
  ),
  'P1.13 migration must be present in the production migration manifest.'
);
...
assert.match(
  ...
  'P1.13 must rebuild ingredient identities after verified promotions.'
);
...
console.log('P1.13 verified exact identity batch five contract passed.');
