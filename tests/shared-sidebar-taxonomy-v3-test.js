...
for (const file of ['registry-v2.js','classification-v2.js','icd-v2.js','urgjencat-v2.js','analizat-v2.js','medical-hub-v2.js']) {
...
for (const file of ['dozologjia.html','protokollet.html','recetat.html','sistemi.html']) {
  const html = read(file);
  assert.match(html, /tailadmin-shell\.js\?v=shell-profile-v4/, `${file}: shared TailAdmin sidebar cache-bust missing`);
}

for (const file of ['index.html','klasifikimi.html','icd.html','urgjencat.html','analizat.html','medical-hub.html']) {
  const html = read(file);
