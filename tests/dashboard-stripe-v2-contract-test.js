...
for (const file of pages) {
  ...
  const stripeIndex = styles.findIndex(href => /drx-dashboard-stripe\.css\?v=drx-dashboard-stripe-v5/.test(href));
...
assert.match(icdHtml, /drx-dashboard-stripe\.css\?v=drx-dashboard-stripe-v5/);
...
for (const [htmlFile, cssFile, jsFile, markerName] of [
  ...
  assert.ok(/drx-dashboard-stripe\.css\?v=drx-dashboard-stripe-v5/.test(styles[1]), `${htmlFile}: shared Stripe shell must load last`);
