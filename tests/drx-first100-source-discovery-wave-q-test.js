...
const prior = [...'abcdefghijklmnop'].flatMap(letter =>
...
for (const row of wave.rows) {
  ...
  assert.equal(Policy.publicationDecision({
    url:row.url,
    documentType:'SmPC',
...
assert.equal(imazol.sourceTier, 'FACHINFO_DE');
assert.equal(imazol.registrationNumber, '49824.00.00');
assert.equal(imazol.documentDate, '2024-06');
assert.match(imazol.atcCode, /D07AA03/);
assert.match(imazol.productName, /Imazol/);
assert.ok(imazol.reviewFlags.includes('professional_product_information_not_regulator'));
assert.ok(imazol.reviewFlags.includes('manual_publication_review_required'));

console.log('DRx first-100 source discovery wave P contract passed.');
