...
const wave = read('data/drx-first100-source-discovery-wave-q-v1.json');
...
const prior = [...'abcdefghijklmnop'].flatMap(letter => read('data/drx-first100-source-discovery-wave-' + letter + '-v1.json').rows);
...
const imazol = wave.rows[0];
assert.equal(imazol.canonicalKey, 'clotrimazolehexamidinediisetionateprednisolone');
assert.equal(imazol.sourceTier, 'KOSOVO_AKPPM');
assert.equal(imazol.registrationNumber, 'MA-0323/06/10/2023');
assert.equal(imazol.atcCode, 'D07AA03');
assert.match(imazol.productName, /Imazol Comp/);
assert.match(imazol.url, /akppm\.rks-gov\.net/);
assert.ok(imazol.reviewFlags.includes('corticosteroid_duration_limit'));
assert.ok(imazol.reviewFlags.includes('clinical_review_required'));

console.log('DRx first-100 official source discovery wave Q contract passed.');
