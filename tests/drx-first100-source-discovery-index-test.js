...
assert.equal(x.verifiedProductSpecific,71);
assert.equal(x.productSelectionRequired,1);
assert.equal(x.verifiedCanonicalSubstances,71);
assert.ok((x.sourceAuthorityCounts.EMC||0)>0);
assert.ok((x.sourceAuthorityCounts.AEMPS_CIMA||0)>0);
assert.equal(x.eligibleRemaining,17);
assert.equal(x.sourceLookupRemaining,16);
