...
assert.equal(x.verifiedProductSpecific,66);
assert.equal(x.productSelectionRequired,1);
assert.equal(x.verifiedCanonicalSubstances,66);
assert.ok((x.sourceAuthorityCounts.EMC||0)>0);
assert.ok((x.sourceAuthorityCounts.AEMPS_CIMA||0)>0);
assert.equal(x.eligibleRemaining,22);
assert.equal(x.sourceLookupRemaining,21);
