...
async function openWorkspace(page) {
  ...
  await expect(html).toHaveAttribute('data-mi-icd-prescription-roundtrip', 'icd-rx-roundtrip-v1');
...
test('workspace source health preserves live metadata, favorites and cached context offline', async ({ page, context }) => {
  ...
  await expect(page.locator('#icdFavoritesToggle')).toBeVisible();
  ...
  await expect(page.locator('#icdFavoritesToggle')).toBeEnabled();
...
test('manual source refresh performs one bounded retry and keeps all ICD layers usable', async ({ page }) => {
  const controls = {};
  await installMetaRoute(page, controls);
  await openWorkspace(page);
  ...
  await expect(page.locator('html')).toHaveAttribute('data-mi-icd-favorites', 'icd-favorites-v1');
  await expect(page.locator('#icdFavoritesToggle')).toBeEnabled();
