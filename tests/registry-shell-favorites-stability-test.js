...
// been defined yet, returns false, throws synchronously, stalls, or is restored
// from BFCache. Every recovery path stays bounded.
...
assert.match(personal, /try \{[\s\S]*?requested = loader\(`personal-view-\$\{activeView\}`\);[\s\S]*?\} catch \{[\s\S]*?personalRuntimeRequested = false;[\s\S]*?schedulePersonalRuntimeRetry\(\)/,
  'synchronous loader errors must release the Favorites latch and retry');
...
const personalPageShow = personal.match(/window\.addEventListener\('pageshow',[\s\S]{0,1900}?\{ passive:true \}\);/)?.[0] || '';
...
assert.match(personalPageShow, /personalRuntimeRequested = false/, 'bfcache restore must always clear stale runtime request state');
assert.match(personalPageShow, /clearPersonalRuntimeRecovery\(\{ resetCount:true \}\)/,
  'bfcache restore must clear frozen retry/watchdog state even if the view did not change');
assert.match(personalPageShow, /requestPersonalRuntime\(\)/, 'bfcache restore must retry full runtime for a personal view');
assert.match(personal, /document\.addEventListener\('visibilitychange',[\s\S]*?document\.visibilityState !== 'visible'[\s\S]*?personalRuntimeRequested = false;[\s\S]*?requestPersonalRuntime\(\)/,
  'returning to a visible tab must recover an unfinished Favorites handoff');
...
// only in the short boot window, and suppress only immediate reload loops. The
// guard expires so later deployments in the same browser session can still
// activate without requiring a manual hard refresh.
...
assert.match(offline, /CONTROLLER_RELOAD_GUARD_MS = 30000/);
assert.match(offline, /Date\.now\(\) - RUNTIME_STARTED_AT > CONTROLLER_BOOT_RELOAD_WINDOW_MS/);
assert.match(offline, /JSON\.parse\(sessionStorage\.getItem\(CONTROLLER_RELOAD_KEY\) \|\| 'null'\)/);
assert.match(offline, /previous\?\.token === token && now - Number\(previous\.at \|\| 0\) < CONTROLLER_RELOAD_GUARD_MS/);
assert.match(offline, /sessionStorage\.setItem\(CONTROLLER_RELOAD_KEY, JSON\.stringify\(\{ token, at:now \}\)\)/);
...
console.log('✓ Registry shell + Favorites stability v2 contract passed');
