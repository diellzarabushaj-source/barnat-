...
assert.equal(snapshot.counts.batch2ArchiveSnapshots, 25);
assert.equal(snapshot.counts.batch2ArchiveCiVerified, 25);
...
assert.equal(snapshot.counts.first100VerifiedProductSources, 87);
assert.equal(snapshot.counts.first100SourceDiscoveryRemaining, 0);
assert.equal(snapshot.counts.first100SourceSectionVerificationPending, 0);
assert.equal(snapshot.counts.first100EffectiveSourceDiscoveryEligible, 87);
assert.equal(snapshot.counts.first100ProductSelectionPending, 0);
assert.equal(snapshot.first100ProgressPct.verifiedProductSources, 100);
assert.equal(snapshot.batch2ProgressPct.structuredEvidence, 100);
assert.equal(snapshot.batch2ProgressPct.archiveHashes, 100);
assert.equal(snapshot.batch2ProgressPct.archiveCiVerified, 100);
...
assert.equal(snapshot.gates.archiveWorkflowRunObserved, true);
assert.equal(snapshot.gates.archiveCiEvidenceComplete, true);
assert.equal(snapshot.gates.archiveEvidenceMaterialized, true);
assert.equal(snapshot.gates.archiveEvidenceLocation, 'repo_attestation_and_supabase_v3_source_snapshots');
assert.equal(snapshot.architecture.v3Applied, true);
assert.equal(snapshot.gates.supabaseLiveAvailable, true);
