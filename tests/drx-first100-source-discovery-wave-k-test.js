...
const prior = [...'abcdefghij'].flatMap(letter =>
...
const etacizins = wave.rows[0];
assert.equal(etacizins.canonicalKey, 'aethacizine');
assert.equal(etacizins.sourceTier, 'EU_NATIONAL');
assert.equal(etacizins.registrationNumber, '98-0371');
assert.equal(etacizins.documentDate, '2024-10-03');
assert.equal(etacizins.atcCode, 'C01BC');
assert.match(etacizins.productName, /Etacizīns 50 mg/);
assert.match(etacizins.url, /\/98-0371\/smpc$/);
assert.ok(etacizins.reviewFlags.includes('high_risk_class_ic_antiarrhythmic'));
assert.ok(etacizins.reviewFlags.includes('clinical_review_required'));
