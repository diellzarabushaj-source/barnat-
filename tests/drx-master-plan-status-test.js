...
assert.equal(tracker.phases.find(p => p.id === 17).status, 'FIRST100_78_OF_89_SOURCE_DISCOVERY_PRODUCTION_PROVENANCE_BLOCKED');
...
assert.equal(tracker.currentExecution.first100VerifiedProductSources, 78);
assert.equal(tracker.currentExecution.first100SourceLookupRemaining, 10);
