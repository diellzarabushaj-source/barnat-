...
assert.equal(tracker.phases.find(p => p.id === 17).status, 'FIRST100_76_OF_89_SOURCE_DISCOVERY_PRODUCTION_PROVENANCE_BLOCKED');
...
assert.equal(tracker.currentExecution.first100VerifiedProductSources, 76);
assert.equal(tracker.currentExecution.first100SourceLookupRemaining, 12);
