...
assert.equal(tracker.phases.find(p => p.id === 17).status, 'FIRST100_83_OF_89_SOURCE_DISCOVERY_PRODUCTION_PROVENANCE_BLOCKED');
...
assert.equal(tracker.currentExecution.first100VerifiedProductSources, 83);
assert.equal(tracker.currentExecution.first100SourceLookupRemaining, 5);
