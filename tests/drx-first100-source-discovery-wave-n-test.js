...
const wave = read('data/drx-first100-source-discovery-wave-n-v1.json');
...
const prior = [...'abcdefghijklm'].flatMap(letter => read('data/drx-first100-source-discovery-wave-' + letter + '-v1.json').rows);
...
const hemorzon = wave.rows[0];
assert.equal(hemorzon.canonicalKey, 'benzocainehydrocortisonetetracycline');
assert.equal(hemorzon.sourceTier, 'EU_NATIONAL');
assert.equal(hemorzon.registrationNumber, '13384/2020/01-02-03');
assert.equal(hemorzon.documentDate, '2020-08-10');
assert.equal(hemorzon.atcCode, 'C05AA01');
assert.match(hemorzon.productName, /Hemorzon 20 mg\/10 mg\/8,3 mg/);
assert.match(hemorzon.url, /anm\.ro/);
assert.ok(hemorzon.reviewFlags.includes('rectal_route_specific'));
assert.ok(hemorzon.reviewFlags.includes('clinical_review_required'));

console.log('DRx first-100 official source discovery wave N contract passed.');
