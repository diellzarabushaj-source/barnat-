...
const dafiro = wave.rows.find(x=>x.canonicalKey==='amlodipinehydrochlorothiazidevalsartan');
assert.equal(dafiro.section41Present,true);
assert.equal(dafiro.section42Present,true);
assert.match(dafiro.productName,/Dafiro HCT/);
assert.match(dafiro.documentDateSource,/EMA Exforge HCT Product Information/);
