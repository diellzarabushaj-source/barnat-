...
const mobileCss = read('registry-table-tools.css');
const desktop = read('registry-desktop-lite.js');
const css = read('registry-table-tools.css');
const fullTextCss = read('registry-table-tools.css');
...
assert.match(index,/registry-table-tools\\.css\\?v=/,'single registry stylesheet authority must be wired');\nassert.doesNotMatch(index,/registry-(?:unified-table|full-text-expansion|mobile-lite)\\.css/,'legacy registry stylesheets must not be wired separately');
...
assert.match(css,/consolidated from registry-mobile-lite\\.css/,'mobile lightweight styles must be materialized in the final stylesheet');
...
assert.ok(index.indexOf('tailadmin-professional.css') < index.indexOf('registry-table-tools.css'),'the single registry stylesheet must remain after TailAdmin professional');
