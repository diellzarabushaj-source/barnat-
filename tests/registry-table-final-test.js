...
assert.match(index,/registry-runtime-loader\.js\?v=20260801-3/,'first-interaction registry runtime loader must be wired');
...
assert.match(read('tests/registry-fast-start-test.js'),/authenticated first-interaction gate v3/,'fast-start audit must cover interaction gate v3');
assert.match(read('tests/registry-interaction-resilience-test.js'),/first-interaction gate v3/,'interaction resilience audit must cover interaction gate v3');
assert.match(read('tests/registry-main-thread-deep-audit-test.js'),/first-interaction gate v3 and main-thread deep audit passed/,'main-thread audit must cover interaction gate v3');
assert.match(read('tests/runtime-asset-version-audit-test.js'),/first-interaction gate v3 and idle dosage asset audit passed/,'runtime version audit must cover interaction gate v3');
...
assert.match(loader,/registry-runtime-loader-v3/,'loader must expose the current interaction gate version');
assert.match(loader,/FIRST_INTERACTION_FALLBACK_MS = 5000/,'registry must retain a bounded automatic fallback without competing with the first interaction');
assert.match(loader,/POST_INTERACTION_GRACE_MS = 320/,'the first interaction must finish before registry startup');
assert.match(loader,/INTERACTION_EVENTS = \['pointerdown', 'keydown', 'touchstart'\]/,'pointer, keyboard and touch interactions must be supported');
assert.match(loader,/handleFirstInteraction[\s\S]*scheduleRuntime\(POST_INTERACTION_GRACE_MS\)/,'first interaction must schedule the heavy runtime asynchronously');
...
console.log('Unified final registry table, filter, expansion, verification, first-interaction startup v3 and responsive audit passed.');
