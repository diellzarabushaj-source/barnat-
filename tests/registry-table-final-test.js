...
assert.match(index,/registry-runtime-loader\.js\?v=20260801-4/,'completed-interaction registry runtime loader must be wired');
...
assert.match(read('tests/registry-fast-start-test.js'),/completed first-interaction gate v4/,'fast-start audit must cover completed interaction gate v4');
assert.match(read('tests/registry-interaction-resilience-test.js'),/completed first-interaction gate v4/,'interaction resilience audit must cover completed interaction gate v4');
assert.match(read('tests/registry-main-thread-deep-audit-test.js'),/completed first-interaction gate v4 and main-thread deep audit passed/,'main-thread audit must cover completed interaction gate v4');
assert.match(read('tests/runtime-asset-version-audit-test.js'),/completed first-interaction gate v4 and idle dosage asset audit passed/,'runtime version audit must cover completed interaction gate v4');
...
assert.match(loader,/registry-runtime-loader-v4/,'loader must expose the current completed interaction gate version');
assert.match(loader,/FIRST_INTERACTION_FALLBACK_MS = 15000/,'registry must retain a bounded automatic fallback without competing with the first interaction');
assert.match(loader,/POST_INTERACTION_GRACE_MS = 1500/,'the completed interaction must settle before registry startup');
...
console.log('Unified final registry table, filter, expansion, verification, completed first-interaction startup v4 and responsive audit passed.');
