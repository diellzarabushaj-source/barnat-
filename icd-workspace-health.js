(() => {
  ...
  const CACHE_KEY = 'medindex_icd_workspace_health_v1';
  ...
  function renderSource(source, options = {}) {
    ...
    if (status === 'stale') {
      setState('stale', 'Po përdoret kopja e fundit e vlefshme', sourceDetail(source, options), source, { cached:Boolean(options.cached) });
      return;
    }
    if (status === 'live') {
      setState(
  ...
  async function requestMeta(signal, attempt = 0) {
    try {
      ...
      return payload.data.meta.source || { type:'unknown', status:'unknown', loadedAt:new Date().toISOString() };
    } catch (error) {
  ...
  async function refresh({ manual = false, reloadTree = false } = {}) {
    ...
    if (manual) setState('loading', 'Po rifreskohet burimi ICD-10…', 'Po kontrollohet Google Sheet-i publik.', currentSource);
    ...
    pendingRefresh = (async () => {
      ...
      if (currentSource) {
        ...
        setState('error', 'Burimi ICD-10 nuk u verifikua', 'Përdor “Rifresko” për ta provuar përsëri.', null, { error:clean(error?.message) });
      }
      measure();
      return null;
    })().finally(() => {
