(() => {
  ...
  function patientProfile(protocol) {
  ...
  function fallbackReview(protocol) {
    ...
    if (!String(protocol?.patientName || '').trim()) issues.push({ message:'Mungon emri i pacientit.' });
    ...
    if (!protocol?.clinicalReview || !protocol?.reviewedAt) issues.push({ message:'Receta nuk është shënuar dhe datuar si e kontrolluar klinikisht.' });
  ...
  function markup(protocol) {
    ...
    const finalised = review.ok && Boolean(protocol.clinicalReview) && Boolean(protocol.reviewedAt);
    ...
    return `<article class="mi-rx ${finalised ? 'is-final' : 'is-draft'}">
      ...
      <div class="mi-rx-meta">
        <span><i>Pacienti</i>${esc(protocol.patientName || 'Nuk është shënuar')}</span>
        ${protocol.birthDate ? `<span><i>Datëlindja</i>${esc(protocol.birthDate)}</span>` : ''}
        ${protocol.patientId ? `<span><i>Nr. personal / ID</i>${esc(protocol.patientId)}</span>` : ''}
        <span><i>Profili</i>${esc(patientProfile(protocol))}</span>
        ...
        <span><i>Alergjitë</i>${esc(protocol.allergies || 'Nuk janë shënuar')}</span>
        ...
        ${protocol.reviewedBy ? `<span><i>Kontrolluar nga</i>${esc(protocol.reviewedBy)}</span>` : ''}
      ...
      <div class="mi-signature-grid">
        <span><i>Përshkruesi</i><b>Dr. Diellza Rabushaj</b></span>
        <span><i>Nënshkrimi</i><b class="mi-line"></b></span>
        <span><i>Vula</i><b class="mi-line"></b></span>
      </div>
