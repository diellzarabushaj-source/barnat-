(() => {
  ...
  function markup(protocol) {
    return `<article class="mi-rx">
      ...
      <div class="mi-rx-meta"><span><i>Indikacioni</i>${esc(protocol.indication || 'Nuk është shënuar')}</span><span><i>Grupi / kushtet</i>${esc(protocol.population || 'Nuk është shënuar')}</span><span><i>Pacienti</i>${esc(patientSummary(protocol))}</span><span><i>Versioni</i>${esc(protocol.version || 'Pa version')}</span></div>
      ...
      <footer>Auto-fill është pikënisje e editueshme · Skema aplikohet vetëm pas konfirmimit · Verifikohet klinikisht para përdorimit individual.</footer>
    </article>`;
  }
  ...
  function ensure() {
    ...
    overlay.addEventListener('click', async event => {
      ...
      if (action === 'print') {
        const protocol = get(current); const popup = window.open('', '_blank');
        if (!protocol || !popup) return;
        popup.document.write(`<meta charset="utf-8"><title>${esc(protocol.name)}</title><style>${PRINT}</style>${markup(protocol)}<script>onload=()=>print()<\/script>`);
