-- Rollback DRx Phase 9G.
-- Do not reopen Phase 9 after Phase 10 has begun.
...
begin
  if exists (
    ...
    raise exception 'Phase 9G rollback blocked: Phase 10 migration history exists';
  end if;
end
$$;

-- Restore the fail-closed Phase 9F status before removing its QA ledger dependency.
...
drop function if exists drx_dose.guard_phase9_frontend_qa_evidence_immutable_v1();

drop table if exists drx_dose.phase9_frontend_qa_evidence_v1;
