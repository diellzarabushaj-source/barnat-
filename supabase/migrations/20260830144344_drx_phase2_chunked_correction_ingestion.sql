create or replace function public.drx_registry_begin_correction_import_v1(
  p_source_ref text,
...
create or replace function drx_raw.ingest_registry_correction_item_v1(
  p_batch_id uuid,
  p_item jsonb
)
returns text
...
declare
  ...
  v_top_count integer;
begin
  select batch_kind,status into v_batch_kind,v_batch_status
  from drx_raw.registry_import_batches_v1
  where batch_id=p_batch_id;

  if v_batch_kind is null then
    raise exception 'Unknown correction batch %',p_batch_id;
  end if;
  if v_batch_kind<>'CORRECTION_SHEET' then
    raise exception 'Batch % is not a correction batch',p_batch_id;
  end if;
  if v_batch_status='FAILED' then
    raise exception 'Correction batch % is failed and immutable',p_batch_id;
  end if;

  v_row_number := nullif(p_item->>'source_row_number','')::integer;
  v_payload := p_item->'raw_payload';
  v_metadata := jsonb_build_object('evidence_urls',coalesce(p_item->'evidence_urls','[]'::jsonb));
  ...
  select raw_sha256,source_row_id into v_existing_hash,v_source_row_id
  ...
  if v_existing_hash is not null then
    if v_existing_hash<>v_hash then
      raise exception 'Immutable correction source row conflict in batch %, row %',p_batch_id,v_row_number;
    end if;
  else
    if v_batch_status<>'OPEN' then
      raise exception 'Cannot append a new correction row to finalized batch %',p_batch_id;
    end if;
    ...
    ) values (p_batch_id,v_row_number,v_correction_id,v_payload,v_metadata,v_hash)
  ...
  if v_batch_status<>'OPEN' then
    raise exception 'Cannot append correction % to finalized batch %',v_correction_id,p_batch_id;
  end if;
  ...
  from jsonb_array_elements_text(coalesce(p_item->'evidence_urls','[]'::jsonb))
...
create or replace function public.drx_registry_append_corrections_v1(
  p_batch_id uuid,
  p_rows jsonb
)
returns jsonb
...
begin
  ...
  if jsonb_array_length(p_rows)>25 then
    raise exception 'Correction chunks are limited to 25 rows per RPC';
  end if;

  for v_item in select value from jsonb_array_elements(p_rows)
  loop
    v_result := drx_raw.ingest_registry_correction_item_v1(p_batch_id,v_item);
...
create or replace function public.drx_registry_finalize_correction_import_v1(
  p_batch_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  if v_kind<>'CORRECTION_SHEET' then
    raise exception 'Batch % is not a correction batch',p_batch_id;
  end if;
  if v_status='FAILED' then
    raise exception 'Correction batch % is failed',p_batch_id;
  end if;
  ...
  join drx_raw.registry_correction_source_rows_v1 s on s.source_row_id=c.source_row_id
  where s.batch_id=p_batch_id;

  if v_source_rows<>v_expected or v_corrections<>v_expected then
    raise exception 'Correction batch % incomplete: expected %, source rows %, corrections %',
      p_batch_id,v_expected,v_source_rows,v_corrections;
  end if;

  v_base := public.drx_registry_finalize_import_v1(p_batch_id);
  return v_base || jsonb_build_object('corrections',v_corrections);
end;
$$;

revoke all on function public.drx_registry_begin_correction_import_v1(text,text,text,integer) from public,anon,authenticated;
revoke all on function public.drx_registry_append_corrections_v1(uuid,jsonb) from public,anon,authenticated;
revoke all on function public.drx_registry_finalize_correction_import_v1(uuid) from public,anon,authenticated;
revoke execute on function drx_raw.ingest_registry_correction_item_v1(uuid,jsonb) from public,anon,authenticated;

grant execute on function public.drx_registry_begin_correction_import_v1(text,text,text,integer) to service_role;
grant execute on function public.drx_registry_append_corrections_v1(uuid,jsonb) to service_role;
grant execute on function public.drx_registry_finalize_correction_import_v1(uuid) to service_role;

comment on function public.drx_registry_append_corrections_v1(uuid,jsonb) is
  'DRx Phase 2 service-only chunked correction ingestion. Maximum 25 rows per RPC to remain below PostgREST statement timeout.';
