...
-- version: 20260827145654
-- name: p1_duplicate_component_identity_collapse

alter table public.product_ingredients_v1
  add column if not exists source_occurrence_count integer not null default 1,
  add column if not exists source_terms text[] not null default '{}'::text[];

update public.product_ingredients_v1
set source_occurrence_count=1,
    source_terms=case
      when cardinality(source_terms)=0 then array[source_term]
      else source_terms
    end
where source_occurrence_count<>1 or cardinality(source_terms)=0;

alter table public.product_ingredients_v1
  drop constraint if exists product_ingredients_v1_source_occurrence_count_check;
alter table public.product_ingredients_v1
  add constraint product_ingredients_v1_source_occurrence_count_check
...
alter table public.product_ingredient_resolution_v1
  add column if not exists source_component_count integer,
  add column if not exists duplicate_component_count integer not null default 0;

alter table public.product_ingredient_resolution_v1
  drop constraint if exists product_ingredient_resolution_v1_source_counts_check;
alter table public.product_ingredient_resolution_v1
  add constraint product_ingredient_resolution_v1_source_counts_check
  check (
    duplicate_component_count >= 0
    and (source_component_count is null or source_component_count >= 0)
    and (
      source_component_count is null
      or expected_component_count is null
      or source_component_count >= expected_component_count
    )
  );
...
create or replace view public.medindex_p1_safe_delimiter_v2
with (security_invoker = true) as
select
  source_drug_id,
  count(*)::integer as identity_count,
  sum(source_occurrence_count)::integer as source_component_count,
  (sum(source_occurrence_count)-count(*))::integer as duplicate_component_count
from public.medindex_p1_resolved_delimiter_parts_v2
group by source_drug_id;

revoke all on public.medindex_p1_resolved_delimiter_parts_v2,
...
begin
  ...
  if n >= 276 then
    raise exception 'Duplicate component collapse did not improve review coverage: %',n;
  end if;
end $$;
