-- DRx Phase 8U: align the paracetamol clinical reference with exact-product age bands.
-- The old eMC snapshot remains immutable provenance. The reference row is reset
-- to PENDING/MISSING and must be fetched, hashed, ingested and reviewed again.
...
begin
  ...
  if not found then
    raise exception 'Phase 8U blocked: paracetamol clinical reference row not found';
  end if;

  if v_row.source_key<>'emc-13495-phase8-clinical-ref'
     or lower(btrim(v_row.source_url))<>'https://www.medicines.org.uk/emc/product/13495/smpc'
    ...
    raise exception 'Phase 8U blocked: current paracetamol reference has changed or has been reviewed';
  end if;

  update drx_dose.phase8_pilot_clinical_references_v1
  set source_key='emc-13494-phase8-clinical-ref',
      source_url='https://www.medicines.org.uk/emc/product/13494/smpc',
