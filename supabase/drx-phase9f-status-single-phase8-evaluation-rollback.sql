-- Rollback DRx Phase 9F.
-- Restore the Phase 9E status definition.
-- DRx Phase 9E: backend/frontend-foundation status gate.
-- Read-only gate. Phase 9 remains on V2 fallback until Phase 10.
...
with metrics as (
  select
    coalesce((public.drx_phase8_status_v1()->>'exit_gate_pass')::boolean,false) phase8_closed,
    coalesce((public.drx_phase8_status_v1()->>'v2_runtime_preserved')::boolean,false) v2_runtime_preserved,
    coalesce((public.drx_phase8_status_v1()->>'v3_cutover_enabled')::boolean,false) v3_cutover_enabled,
    exists (
      select 1
