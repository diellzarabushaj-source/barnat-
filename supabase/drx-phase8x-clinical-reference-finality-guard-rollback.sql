-- Rollback Phase 8X finality guard.
-- Refuses rollback once a human clinical decision exists.
...
begin
  if exists (
    ...
    raise exception 'Phase 8X rollback blocked: reviewed clinical references must remain immutable';
  end if;
end $$;

drop trigger if exists aa_drx_phase8_clinical_reference_finality_guard
  on drx_dose.phase8_pilot_clinical_references_v1;
drop function if exists drx_dose.guard_phase8_clinical_reference_finality_v1();
